                    ...
                    <div class="panel-body collapse in">
                        <div class="row">
                            ...
                            <div class="form-group" style="text-align: center">
                                ...
                                <div class="col-md-3">
                                    <input type="text" tabindex="3" name="prem_date" required="required" class="form-control date" data-inputmask="'mask':'99/9999'" id="prem_date" value="<?php echo date('m/Y');?>" maxlength="7">
                                </div>
                            </div>
                            <div class="form-group" style="text-align: center">
...
<script type="application/javascript">
    $(function() {
        ...
        $("#ins_submit").click(function(){
            var prem_date = $('#prem_date').val();
            if(prem_date != '')
            {
                var date = new Date();
                var day = date.getDate();
                prem_date = day+'/'+prem_date;
                prem_date = process(prem_date);

                //setHours removes time from date
                today_date = new Date().setHours(0,0,0,0);

                if(prem_date >= today_date)
                {
                    ...
                    if ((famName != null && famName.length != 0) || (client_id != null && client_id.length != 0)) {
                        $.ajax({
                            url: "<?php echo site_url('broker/Insurances/get_prem_cal_report');?>",
                            type: 'post',
                            data: {'famName': famName, client_id: client_id, prem_date: $('#prem_date').val()},
                            dataType: 'json',
                            success: function(data)
                            {
                                if(data['Status'])
                                {
                                    window.open("<?php echo site_url('broker/Reports/get_premium_calendar_report');?>", '_blank')
                                }
                                else
                                {
                ...
                {
                    bootbox.alert("Please Select Today's or Later Date to view Reports");
                }
            }
            else
            {
