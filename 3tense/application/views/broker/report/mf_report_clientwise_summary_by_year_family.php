...
if(empty($mf_rep_data))
{
  ...
  $sParNavindex=0;$gTotalPurNav=0;$gPurNavIndex=0;$per=0;$year='';
  $livexirr=0;
  $brokerId=0;
  // echo"<pre>" ;
  // print_r($mf_rep_data);
  ...
  $title = '<div class="title row">Client Wise Summary Mutual Fund Portfolio of '.$familyName.' Family By Year</div><br/>';
  ...
  $arrPrintedClientNames = array();
$blnHeaderGenerated = false;
$arrHeaderGenerated = array();
  ...
  $year = explode('/',$mf_rep_data[0]->purchase_date)[2];
  $newYear = true;
  // $i=0;
  foreach($mf_rep_data as $row)
  {
      ...
      $rowYear = explode('/',$mf_rep_data[$i]->purchase_date)[2]; 
      if($clientName != $mf_rep_data[$i]->client_name || $year != $rowYear)
      {
          $schemeFoot = true;
      }
      if($clientName != $mf_rep_data[$i]->client_name || $year != $rowYear)
      ...
      if($newYear)
      {
        $strYear = '<tr nobr="true">
                    <td class="border" colspan="17"><h4>'.$rowYear.'</h4></td>
                </tr>';
       // $newClient = false;
        //$newYear = false;
        }
      if($newClient && $newYear)
      {
        if(!array_key_exists($mf_rep_data[$i]->client_name, $arrPrintedClientNames))
        {
             $html .= '<tr nobr="true">
                    <td class="no-border client-name2" colspan="4"><h4>'.$mf_rep_data[$i]->client_name.'</h4></td>
                </tr>';
             $arrPrintedClientNames[$mf_rep_data[$i]->client_name] = '1';
        }
       ...
       if($header)
       {
           if(!array_key_exists($mf_rep_data[$i]->client_name, $arrHeaderGenerated))
           {
           ...
           $arrHeaderGenerated[$mf_rep_data[$i]->client_name] = 1;
           
           }
           $blnHeaderGenerated = true;
            $header = false;
            $content = true;
        } $html = $html . $strYear;
      if($content)
        ...
        // $schemeName = $row->mf_scheme_name;
          $clientName= $mf_rep_data[$i]->client_name;
          $year = explode('/',$mf_rep_data[$i]->purchase_date)[2]; 
          $i++;
