...
// echo"</pre>";
 ...
 else
 {
     ...
     $title = '<div class="title row">Capital Gain Mutual Fund Portfolio of '.$familyName.' Family</div><br/>';
     ...
     $html = '<table border="0" cellpadding="4" style="text-align:center; border-width:0px;">
        ...
        $fPur_amt='';$fDiv_amt='';$fUnit='';$fSale_amt='';$fDiv_r2='';$fPayout='';$fST_profit='';$fST_loss='';$fLT_profit='';$fLT_loss='';
        $fTotalCagr1=0;$fTotalAbs1=0;$fTotalCagr2=0;$fTotalCagr=0;$fTotal_abs=0;
   ...
   foreach($capital_gain as $row)
   {
    ...
    {
        $familyFoot = false;
        $html.='<tr nobr="true"><td class="border" colspan="6"></td>';
        ...
        $sPurAmt=0;$sDivAmt;$unit;$sSale_amt='';$gPurAmt=0;$gDivAmt;$unit;$gSale_amt='';
        $sPur_amt=0;$sDiv_amt=0;$sUnit=0;$sSale_amt=0;$sDiv_r2=0;$sPayout=0;$sST_profit=0;$sST_loss=0;$sLT_profit=0;$sLT_loss=0;
        $cagr1=0;$cagr2=0;$abs1=0;$sTotalCagr1=0;$sTotalAbs1=0;$sTotalCagr2=0;$sTotalCagr=0;$sTotal_abs=0;
        $gPur_amt=0;$gDiv_amt=0;$gUnit=0;$gSale_amt=0;$gDiv_r2=0;$gPayout=0;$gST_profit=0;$gST_loss=0;$gLT_profit=0;$gLT_loss=0;
        $gTotalCagr1=0;$gTotalAbs1=0;$gTotalCagr2=0;$gTotalCagr=0;$gTotal_abs=0;
    ...
    {
        $html .= '<tr nobr="true">
                    <td class="no-border client-name2" colspan="12">'.$row->client_name.'</td>
                </tr>';
        $newClient = false;
    }
    ...
    //  $clientName=$row->client_name;
        ...
        $fPur_amt=$fPur_amt+$row->purchase_amount;
        $fDiv_amt=$fDiv_amt+$row->div_amount;
        $fUnit=$fUnit+$row->units;
        $fSale_amt=$fSale_amt+$row->sale_amount;
        $fDiv_r2=$fDiv_r2+$row->div_r2;
        $fPayout=$fPayout+$row->payout;
        ...
        if($fPur_amt!= 0)
        {
            $fTotal_abs = round(($fTotalAbs1 / ($fPur_amt+$fDiv_amt)), 2);
        }
       else {
            $fTotal_abs = 0;
        }
        ...
        $html.='
              ...
              if($row->transaction_day<365)
              {
                if($row->gain>0)
                {
                    ...
                    $fST_profit=$fST_profit+$row->gain;
                }
                else {
                    $sST_loss=$sST_loss+$row->gain;
                    $gST_loss=$gST_loss+$row->gain;
                    $fST_loss=$fST_loss+$row->gain;
                }
                ...
                if($row->gain>0)
                {
                    ...
                    $fLT_profit=$fLT_profit+$row->gain;
                  }
                  else {
                    $sLT_loss=$sLT_loss+$row->gain;
                    $gLT_loss=$gLT_loss+$row->gain;
                    $fLT_loss=$fLT_loss+$row->gain;
                  }
                $html.='<td class="border amount"></td>
                        <td class="border amount"></td>';
                $html.=$row->gain>0?'<td class="border amount">'.$this->common_lib->moneyFormatIndia(round($row->gain)).'</td><td class="border" ></td>':'<td class="border"></td><td class="border amount">'.$this->common_lib->moneyFormatIndia(round($row->gain)).'</td>';
    ...
    $html .= '<tr nobr="true"><td class="border dataTotal" colspan="6">Family Total</td>';
             $html.=!empty($fPur_amt)?'<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fPur_amt)).'</td>':'<td class="border"></td>';
             $html.=!empty($fDiv_amt)?'<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fDiv_amt)).'</td>':'<td class="border"></td>';
             $html.='<td class="dataTotal border amount" colspan="2"></td>';
             $html.=!empty($fUnit)?'<td class="dataTotal border amount">'.$fUnit.'</td>':'<td class="border"></td>';
             $html.=!empty($fSale_amt)?'<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fSale_amt)).'</td>':'<td class="border"></td>';
             $html.='
             <td class="dataTotal border amount"></td>
             <td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fST_profit)).'</td>
             <td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fST_loss)).'</td>
             <td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fLT_profit)).'</td>
             <td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fLT_loss)).'</td>';
             $html.=!empty($fDiv_r2)?'<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fDiv_r2)).'</td>':'<td class="border"></td>';
             $html.=!empty($fPayout)?'<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia(round($fPayout)).'</td>':'<td class="border"></td>';
             $html.='<td class="dataTotal border amount">'.round($fTotalCagr,2).'</td>
             <td class="dataTotal border amount">'.round($fTotal_abs,2).'</td>
             </tr>
             <tr>
                 <td colspan="22" class="no-border info dataTotal">
                    Overall Portfolio Weighted Avg. CAGR: '.$fTotalCagr.' | Weighted Avg. Abs Return: '.$fTotal_abs.'
                 </td>
             </tr>
             <tr nobr="true" border="0"><td colspan="3" border="0" class="default no-border"><br/></td></tr>
             <tr nobr="true" border="0"><td colspan="3" border="0" class="default no-border"><br/></td></tr>
             ...
             <form action="#" id="report_form" method="post" class="form-horizontal row-border">
                 <div class="panel panel-midnightblue">
                     <div class="panel-gray panel-body collapse in panel-noborder">
                         ...
                         <input type="hidden" name="report_name" id="reportName" value="<?php echo $familyName;?> Capital Gain  Portfolio" />
                     </div>
                 </div>
             </form>
         </div>
