...
else
{
    ...
    $title = '<div class="title row">Client Wise Summary Mutual Fund Portfolio of '.$clientName.'</div><br/>';
    // add client info to page
    $html = '<table border="0" cellpadding="4" style="text-align:center; border-width:0px;"><tbody>';
    $header = true; $content = false; $schemeFoot = false;$schemeFoot=false;
    ...
    foreach($mf_rep_data as $row)
    {
        ...
        if($header)
        {
            if($scheme_group_type_show)
            {
                if($is_first_row)
                {
                $html .= '<tr nobr="true"><th colspan="17" style="    border-color: white;">&nbsp;</th></tr>';
                }
                
                $html .= '<tr nobr="true" class="head-row" style="background-color:#76c4ed">';
                $html .= '<th colspan="7" style="font-size:18px">'.$scheme_group_type_name.'</th></tr>';
                $is_first_row=true;
            ...
            $html .= '<tr nobr="true" class="head-row">';
                        ...
                        {
                            $html .= '<th width="130">Scheme Name</th>';
                        }
