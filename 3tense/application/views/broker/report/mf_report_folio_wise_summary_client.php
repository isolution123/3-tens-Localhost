...
$clientName=$folio_summary['0']->client_name;
$title = '<div class="title row">Folio Wise Summary Mutual Fund Portfolio Of '.$clientName.'</div><br/>';
...
$data.='<tr  nobr="true" class="head-row">
                        ...
                        <th width="88">Folio Number</th>
                        <th width="88">Invest Cost</th>
                        <th width="88">Div Amount</th>
                        <th width="88">Live Unit</th>
      ...
      if($folio_summary)
      {
        $f_pur_amt=0;$f_div_amount=0;$f_live_unit=0;$f_current_value=0;$f_div_r2=0;$f_div_payout=0;$f_total=0;$TotalCagrForXirr=0;
        ...
        foreach ($folio_summary as $rs)
        {
         ...
         $data.='<td class=" border amount ">'.$this->common_lib->moneyFormatIndia(round($total )).'</td>
            ...
            $data.='<tr>
                        <td class="border dataTotal" colspan="2">Total</td>
                        <td class="border amount dataTotal">'.$this->common_lib->moneyFormatIndia(round($f_pur_amt)).'</td>
                        <td class="border amount dataTotal">'.$this->common_lib->moneyFormatIndia(round($f_div_amount)).'</td>
                        <td class="border amount dataTotal">'.sprintf("%.2f",$f_live_unit).'</td>
                        <td class="border amount dataTotal">'.$this->common_lib->moneyFormatIndia(sprintf("%.2f",$f_current_value)).'</td>
                        <td class="border amount dataTotal">'.$this->common_lib->moneyFormatIndia(round($f_div_r2)).'</td>';
                        if($f_div_payout==0|| empty($f_div_payout))
                        {
                            $data.='<td class="border amount"></td>';
                        }else {
                            $data.='<td class="border amount">'.$this->common_lib->moneyFormatIndia(round($f_div_payout)).'</td>';
                        }

                        $data.='<td class="border amount dataTotal">'.$this->common_lib->moneyFormatIndia(round($f_total )).'</td>
                        <td class="border amount dataTotal">'.$sTotalCagr.'</td>
                        <td class="border amount dataTotal">'.$sTotal_abs.'</td>
                        </tr></table><br>';
                        $html.=$data;
         }
         $fTotal=$f_total;
            ...
            <form action="#" id="report_form" method="post" class="form-horizontal row-border">
                <div class="panel panel-midnightblue">
                    <div class="panel-gray panel-body collapse in panel-noborder">
                        ...
                        <input type="hidden" name="name" id="name" value="<?php echo $folio_summary[0]->client_name;?>" />
                        <input type="hidden" name="titleData" id="titleData" />
                        <input type="hidden" name="htmlData" id="htmlData" />
                        <input type="hidden" name="report_name" id="reportName" value="<?php echo $folio_summary[0]->client_name;?> Mutual Fund Portfolio" />
                    </div>
                </div>
            </form>
        </div>
