<div id="page-content" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">
    <div id='wrap'>
        ...
        <div class="container">
            <form action="#" id="ledger_report_form" method="post" class="form-horizontal row-border">
                <div class="panel panel-midnightblue">
                    <div class="panel-heading">
                        <h4>Ledger Report Info</h4>
                    </div>
                    <div class="panel-body collapse in">
                        <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-3">
                                    <select name="InvestmentType" class="form-control" id="InvestmentType" style="width: 100%" tabindex="5">
                                        <option value="0" selected> All</option>
                                        <option value="1">Mutual Fund</option>
                                        <option value="2">Insurances</option>
                                        <option value="3">Equity</option>
                                        <option value="4">Commodity</option>
                                        <option value="5">Real Estate</option>
                                        <option value="6">Fixed Deposite</option>
                                    </select>
                                </div>
                            ...
                            <div class="form-group" style="text-align: center">
                                <button type="button" class="btn btn-success ladda-button" tabindex="6" id="ledger_submit" data-style="expand-right"><i class="fa fa-file-text-o"></i> <span class="ladda-label">Generate Report</span></button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="application/javascript">
    $(function() {
        ...
        $("#ledger_submit").click(function(){
            ...
            var to = $('#to_date').val();
            var investment_type=$('#InvestmentType').val();
            if ((famName != null && famName.length != 0) || (client_id != null && client_id.length != 0)) {
                if((from != null && from.length != 0) && (to != null && to.length != 0)) {
                    $.ajax({
                        url: "<?php echo site_url('broker/Final_reports/get_ledger_report');?>",
                        type: 'post',
                        data: {'famName': famName, client_id: client_id, from_date: from, to_date: to,'investment_type':investment_type},
                        dataType: 'json',
                        success: function(data)
                        {
                            if(data['Status'])
                            {
                                window.open("<?php echo site_url('broker/Reports/get_ledger_report');?>", '_blank')
                            }
                            else
                            {
                                console.log(data);
