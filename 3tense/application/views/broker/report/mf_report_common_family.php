...
if($mf_summary_client)
{
    ...
    $total=0;$profit=0;$amt_totals=0;$units_totals=0;$cur_value_total=0;$div_red_total=0;$div_payout_total=0;$profit_total=0;$cagr_total=0;$abs_total=0;
        ...
        $summary_portfolio.='<tr>
                              ...
                              <td class=" border dataTotal amount">'.$this->common_lib->moneyFormatIndia(sprintf("%.2f",$cur_value_total)).'</td>';
 ...
 $summary_typewise='<br><table border="0" cellpadding="4" style="text-align:center; border-width:0px;">';
    ...
    $summary_typewise.='<tr>
                            ...
                            if($div_r2_total==0|| empty($div_r2_total))
                            {
                              ...
                              $summary_typewise.='<td class="border amount">'.$this->common_lib->moneyFormatIndia(round($div_r2_total)).'</td>';
                            }
              ...
              foreach ($mf_comman_scheme_summary as $rs)
              {
                ...
                // $client=$mf_comman_scheme_summary[$i]->client_name;
                 $common_scheme_summ.='<tr>
                  ...
                  $common_scheme_summ.='<tr>
                              <td class="border dataTotal"  colspan="1">Total</td>
                              ...
                              $common_scheme_summ.='<td class="border dataTotal amount">'.sprintf("%.4f",$f_live_unit).'</td>
                                                    <td class="border dataTotal amount">'.$this->common_lib->moneyFormatIndia(sprintf("%.2f",$f_current_value)).'</td>';
...
if($mf_summary_net_investment)
{
            ...
            foreach ($mf_summary_net_investment as $rs)
            {
                ...
                if($rs->brokerID=='0004' || $rs->brokerID=='0009' || $rs->brokerID=='0174' || $rs->brokerID=='0196')
                {
                    ...
                    $heading_netinvestment='<div class="title col-md-12 border" width="300px">Net investment of '.$familyName.' Family </div><br>';
                  $net_investmet=$heading_netinvestment;
                  $net_investmet.='<br><table border="1" cellpadding="4" class="col-md-12">';

              $net_investmet.='<tr>
                  <td class="amount" style="border:2px solid white;background-color:green;color: white;text-align: center;font-weight: bolder;padding: 10px;" > Investment<br>'.$this->common_lib->moneyFormatIndia(round($rs->purchase)).'
                  </td>
                  ...
                  <td class="amount" style="border:2px solid white;background-color: red;color: white;text-align: center;font-weight: bolder;padding: 10px;">Switch Out<br>'.$this->common_lib->moneyFormatIndia(round($rs->swo)).'
                  </td>
                  <td class="amount" style="border:2px solid white;background-color: red;color: white;text-align: center;font-weight: bolder;padding: 10px;">Div. Payout<br>';
                  ...
                  $net_investmet.='</td>
                  <td class="amount" style="border:2px solid white;background-color: #4f8edc;color: white;text-align: center;font-weight: bolder;padding: 10px;">
                  Net Investment<br>'.$this->common_lib->moneyFormatIndia(round(($rs->purchase+$rs->swi)- ($rs->payout + $rs->redemption+$rs->swo))).'
                  </td>
                  <td class="amount" style="border:2px solid white;background-color: #4f8edc;color: white;text-align: center;font-weight: bolder;padding: 10px;">
                  Current Value<br>'.$this->common_lib->moneyFormatIndia(round($rs->current_value)).'
                  </td>
                  <td class="amount" style="min-width: 100px;width: 100px;border:2px solid white;background-color: #4f8edc;color: white;text-align: center;font-weight: bolder;padding: 10px;">
                  Net Gain<br>'.$this->common_lib->moneyFormatIndia(round($rs->current_value-(($rs->purchase+$rs->swi)- ($rs->payout + $rs->redemption+$rs->swo)))).'
                  </td>';
                ...
                {
                    $heading_netinvestment='<div class="title col-md-4 border" width="300px">Net investment of '.$mf_rep_data[0]->family_name.' Family </div><br>';
                  $net_investmet=$heading_netinvestment;
                  $net_investmet.='<br><table border="1" cellpadding="4" width="300px" class="col-md-4">';
   ...
   if($mf_comman_scheme_summary)
{
  ...
  $heading='<br/><div class="title col-md-4 border" style="margin-bottom:10px;" >AMC Wise Summary of '.$familyName.' family</div><br>';
  $common_scheme_summ='<br><table border="0" cellpadding="4" style="text-align:center; border-width:100%;" width="370px">';
                       ...
                       if($rs['allocation']==0 || empty($rs['allocation']))
                        {
                            ...
                            $total_allocation = $total_allocation + $rs['allocation'];
