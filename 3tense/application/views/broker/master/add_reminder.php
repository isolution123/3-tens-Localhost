...
<div class="modal fade" id="modal_form1" role="dialog" style="z-index: 9">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
                ...
                <form action="#" id="form2" class="form-horizontal">
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <i>Please check this document uploaded by <b><lable  id="remClientNamec1"></lable></b> in his/her client details</i>
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <div class="modal-footer">
                ...
                <!-- <button type="button" id="btnMail" class="btn btn-sky"><i class="fa fa-envelope"></i> Send Mail</button>
                <button type="button" id="btnSMS" class="btn btn-midnightblue"><i class="fa fa-mobile"></i> Send SMS</button> -->
                <button type="button" class="btn btn-danger" data-dismiss="modal" id="Delete"><i class="fa fa-times"></i> Decline</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
    ...
    $('#btnComp').click(function(){
        ...
        $('#btnCompclient').click(function(){
            ...
            bootbox.confirm('Are you sure you want to Approve This Document?', function(result){
                ...
                {
                    $.ajax({
                        url: '<?php echo site_url('broker/Reminders/approve')?>',
                        type:'post',
                        data: {'remIDc': $('#remIDc').val(),'client_id':$('#client_idc').val(),'type':$('#remMsgc').val(),'client_name':$('#remClientNamec').val()},
                        dataType: 'json',
                        success:function(data)
                        {
                            if(data['status'] == 'success')
                        ...
                        {
                            console.log(data);
                            bootbox.alert('Error while Approving Document Or If Document is Approved Check Client Master');
                        }
...
$('#Delete').click(function(){
  $.ajax({
      url: '<?php echo site_url('broker/Reminders/delete')?>',
      type:'post',
...
////Client Reminder view
function Client_view_reminder(id)
{
    ...
    $.ajax({
        ...
        {
          console.log(data);
            $('#remIDc').val(data.reminder_id);
            $('#remDatec').val(data.reminder_date);
            $('#remSubc').val(data.reminder_type);
            $('#remClientNamec').val(data.client_name);
            $('#remClientNamec1').html(data.client_name).val();
            $('#remMsgc').val(data.reminder_message);
            $('#remarkc').val(data.remark);
    ...
    function load_notif() {
        $.ajax({
            url: '<?php echo site_url('broker/Dashboard/update_notif')?>',
            //type:'post',
            //data: {'clientID': $('#client_id').val(), 'clientName': $("#client_id option:selected").text(), 'msg': $("#message").val(), 'remDate': $("#reminder_date").val(), 'concern_user': $("#concern_user").val()},
            dataType: 'html',
            success:function(data)
