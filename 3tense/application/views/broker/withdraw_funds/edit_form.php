    ...
    <div id='wrap'>
        <div id="page-heading">
            <ol class="breadcrumb">
                ...
                <li><a href="<?php echo base_url('broker/Withdraw_funds');?>">Withdraw Funds</a></li>
                <li class="active">Edit Withdraw Fund</li>
            </ol>
            <h1>Edit Withdraw Fund</h1>
        </div>
        <div class="container">
            <form action="#" id="fund_form" method="post" class="form-horizontal row-border" data-validate="parsley" >
                <input type="hidden" name="withdraw_fund_id" id="withdraw_fund_id" value="<?php echo $fund_data->withdraw_fund_id; ?>" />
                <div class="panel panel-midnightblue control-form">
                    ...
                    <div class="panel-body collapse in">
                        <div class="row">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">Withdraw From</label>
                                <div class="col-md-4 no-border">
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Equity')?'checked="checked"':'';?> id="radio-equity" value="Equity"> Equity</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Mutual Funds')?'checked="checked"':'';?> id="radio-mf" value="Mutual Funds"> Mutual Funds</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Insurance')?'checked="checked"':'';?> value="Insurance"> Insurance</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Fixed Income')?'checked="checked"':'';?> value="Fixed Income"> Fixed Income</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Commodity')?'checked="checked"':'';?> value="Commodity"> Commodity</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Real Estate')?'checked="checked"':'';?> value="Real Estate"> Real Estate</label></div>
                                    <div class="radio block"><label><input type="radio" name="withdraw_from" <?php echo ($fund_data->withdraw_from=='Others')?'checked="checked"':'';?> value="Others"> Others</label></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="panel-heading info-head" id="equity-head" style="display: none;">
                        <h4>Broker Info</h4>
                    </div>
                    <div class="panel-body collapse in info-body" id="equity-body" style="display: none;">
                        <div class="row">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">Broker Name</label>
                                <div class="col-md-4 no-border add-new-btn">
                                    <select name="trading_broker_id" class="populate" id="trading_broker_id" style="width: 80%" tabindex="12">
                                        <option disabled selected value="">Select Broker Name</option>
                                    </select>
                                    <a href="javascript:;" tabindex="13" class="btn btn-xs btn-inverse-alt" onclick="add_trading(true)"><i class="fa fa-plus"></i></a>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div class="col-md-4 add-new-btn">
                                    <select name="client_code" id="client_code" class="form-control" style="width: 80%" tabindex="14">
                    ...
                    <div class="panel-heading info-head" id="mf-head" style="display: none;">
                        <h4>Mutual Fund Info</h4>
                    </div>
                    <div class="panel-body collapse in info-body" id="mf-body" style="display: none;">
                        <div class="row">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">Mutual Fund Type</label>
                                <div class="col-md-4 no-border">
                                    <div class="radio block"><label><input type="radio" name="mf_type" <?php echo ($fund_data->mf_type=='Redemption')?'checked="checked"':'';?> value="Redemption"> Redemption</label></div>
                                    <div class="radio block"><label><input type="radio" name="mf_type" <?php echo ($fund_data->mf_type=='Dividend')?'checked="checked"':'';?> value="Dividend"> Dividend</label></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="col-sm-12 bottom-col">
    <button type="button" id="add" tabindex="34" onclick="addForm('<?php echo base_url("broker/Withdraw_funds/add_form");?>')" data-style="expand-left" class="btn btn-success bottom-btn ladda-button"><i class="fa fa-plus"></i> Add</button>
...
<script type="text/javascript">
    $(function() {
        ...
        $("input:radio[name=withdraw_from]").on('click', function() {
            $(".info-head").hide("fast");
            $(".info-body").hide("fast");
            if($(this).val() != "Equity") {
        ...
        $('#radio-equity').on('click', function() {
            $("#equity-head").slideDown("fast");
            $("#equity-body").slideDown("fast");
            $("#trading_broker_id").parsley('addConstraint',{required: true});
            $("#client_code").parsley('addConstraint',{required: true});
            $("#mf-head").hide("fast");
            $("#mf-body").hide("fast");
        });
        $('#radio-mf').on('click', function() {
            $("#mf-head").slideDown("fast");
            $("#mf-body").slideDown("fast");
            $("#equity-head").hide("fast");
            $("#equity-body").hide("fast");
            $("#trading_broker_id").parsley('removeConstraint','required');
            $("#client_code").parsley('removeConstraint','required');
        });
        ...
        <?php echo (isset($fund_data->withdraw_from) && $fund_data->withdraw_from == 'Equity')?'$("#radio-equity").trigger("click");':'';?>
        <?php echo (isset($fund_data->withdraw_from) && $fund_data->withdraw_from == 'Mutual Funds')?'$("#radio-mf").trigger("click");':'';?>
    ...
    function fund_submit()
    {
        ...
        if(valid && dateValid)
        {
            ...
            $.ajax({
                url: '<?php echo site_url('broker/Withdraw_funds/update_funds');?>',
                type: 'post',
                data: form_data,
                dataType: 'json',
                success:function(data)
    ...
    function delete_withdraw_fund()
    {
        var id = $("#withdraw_fund_id").val();
        if(id != '' && id != null & id != 0) {
                ...
                if(result)
                {
                    ...
                    $.ajax({
                        ...
                        success: function(data)
                        {
                            ...
                            window.location.href = '<?php echo base_url('broker/Withdraw_funds');?>'
                        },
                        error: function (jqXHR, textStatus, errorThrown)
                        {
