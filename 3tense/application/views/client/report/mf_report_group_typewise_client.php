...
else
{
    ...
    $gTotal = 0; $invested_cost_total=0; $gTotalCagr = 0;  $gTotal_abs = 0; $gTotalCagr1 = 0; $gTotalCagr2 = 0; $gTotalAbs1 = 0; $gLiveUnit = 0;
    if(!empty($mf_rep_data))
    ...
    $title = '<div class="title row">Type wise Mutual Fund Portfolio of '.$clientName.'</div><br/>';
    // add client info to page
    $html = '<table border="0" cellpadding="4" style="text-align:center; border-width:0px;"><tbody>';
    $header = true; $content = false; $schemeFoot = false;
    ...
    foreach ($mf_rep_data as $rss)
    {
     ...
     function hasPropertyValue($objects, $property, $value,$property1, $value1){
    foreach ($objects as $object) {
        if (property_exists($object, $property) && $object->{$property} === $value &&  $object->{$property1} === $value1) {
            return true;
        }
    }
    return false;
    ...
    $scheme_group_list = array("Equity", "Hybrid", "Debt"); 
$scheme_group_type_show=true;
$is_first_row=false;
$schemeFoot=false;
$firstClient=false;
foreach ($scheme_group_list as $scheme_group_type_name) {
  
    $scheme_group_type_show=true;
    $header = true;
    $schemeFoot = true; 
     if(!hasPropertyValue($mf_rep_data,'client_name',$clientName,'Scheme_Group_TypeName',$scheme_group_type_name))
            {
                continue;
            }
    foreach($mf_rep_data as $row)
    {
          ...
          if($schemeFoot) 
                {
                  ...
                  $html .= '</tr>
                  <tr>
                      <td colspan="19" class="no-border info dataTotal">
        ...
        if($header)
        {
            if($scheme_group_type_show)
            {
                if($is_first_row)
                {
                $html .= '<tr nobr="true"><th colspan="17" border="0" style="border-color: white;">&nbsp;</th></tr>';
                }
                
                $html .= '<tr nobr="true" class="head-row" style="background-color:#76c4ed">';
                $html .= '<th colspan="7" style="font-size:18px" width="500px">'.$scheme_group_type_name.'</th></tr>';
                $is_first_row=true;
                $scheme_group_type_show=false;
            }
            $html .= '<tr nobr="true" class="head-row">';
                                    
                            $html .= '<th width="65">Scheme Name</th>';
                        
                            $html .= '<th width="55">Folio Number</th>
                            <th width="60">Inception Date</th>
                            <th width="55">Scheme Type</th>
                            <th width="65">Purchase Amount</th>
                            <th width="55">Div Amount</th>
                            <th width="55">Purchase NAV</th>
                            <th width="65">No. of Unit</th>
                            <th width="40">Trans Day</th>
                            <th width="55">Current NAV</th>';
                           
                            $html .= '<th width="65">Current NAV Date</th>';
                        
                            $html .= '<th width="80">Current Value</th>
                            <th width="55">Div R</th>
        ...
        if($row->Scheme_Group_TypeName==$scheme_group_type_name)
        {
            $scheme_group_type_show=true;
        if($content)
        {
          ...
          // }
            ...
            //set cagr1(temp)
            if($row->cagr1 != null)
                $cagr1 = $row->cagr1;//$purAmt * $row->cagr * $row->transaction_day;
            else
                $cagr1 = 0;
            if($row->mf_abs != null)
            {
                $cagr2 = $row->cagr2;//$purAmt * $row->transaction_day;
  ...
  //footer
  // $sTotalNav=$sTotalPurNav/sizeof($mf_rep_data);
    $html.='<tr nobr="true">
            <td class="border normal dataTotal" colspan="4">Total</td>';
            if(!empty($sTotalPurAmt))
                $html .= '<td class="dataTotal border amount">'.$this->common_lib->moneyFormatIndia($sTotalPurAmt).'</td>';
            else
                $html .= '<td class="border amount normal"></td>';
    ...
    //                 $html .= '<td class="dataTotal border">'.$this->common_lib->moneyFormatIndia($gTotalPurAmt).'</td>';
    //             else
    //                 $html .= '<td class="border normal"></td>';
    //
    //             $html .= '<td class="dataTotal border"></td>
    ...
    //                 $html .= '<td class="dataTotal border">'.$this->common_lib->moneyFormatIndia($gTotalCurValue).'</td>';
    //             else
    //                 $html .= '<td class="border normal"></td>';
    //             if(!empty($gTotalDivR))
    //                 $html .= '<td class="dataTotal border">'.$this->common_lib->moneyFormatIndia($gTotalDivR).'</td>';
    //             else
    //                 $html .= '<td class="border normal"></td>';
    //             if(!empty($gTotalDivPay))
    //                 $html .= '<td class="dataTotal border">'.$this->common_lib->moneyFormatIndia($gTotalDivPay).'</td>';
    //             else
    //                 $html .= '<td class="border normal"></td>';
    //             $html .= '<td class="dataTotal border">'.$this->common_lib->moneyFormatIndia($gTotal).'</td>
    //                       <td class="dataTotal border">'.$gTotalCagr.'</td>
    //                       <td class="dataTotal border">'.$gTotal_abs.'</td>
    //                       <td class="border dataTotal">'.round($perTotal).'%'.'</td>
    //                       <td class="border normal"></td>
    //                       </tr>
    //                       <tr>
    //                           <td colspan="18" class="no-border info dataTotal">
  ...
  include 'mf_3d_report_common_client.php';
  $sTotalPurAmt = 0; $sTotalDivAmt = 0; $sTotalPurNav = 0; $sTotalPurNavDivide = 0; $sTotalLiveUnit = 0; $sTotalCurValue = 0; $sTotalDivR = 0; $sTotalDivPay = 0;
...
{
        document.cookie = "client_select_id = " + sel.value;
    ...
    setTimeout(function(){
     ...
     function type_wise_chart(val1,val2,val3,val4,val5,val6,val7,val8,val9,val10,val11,val12)
    {
        
      
        var total_val=val1+val2+val3+val4+val5+val6+val7+val8+val9+val10+val11+val12;
        var per1=((val1*100)/total_val).toFixed(2);
        var per2=((val2*100)/total_val).toFixed(2);
        var per3=((val3*100)/total_val).toFixed(2);
        var per4=((val4*100)/total_val).toFixed(2);
