...
if(empty($eq_rep_data) && empty($eq_values_data)) {
    ...
    $css = '<style type="text/css">
                ...
                .amount { text-align:right;padding-right:10px; text-indent: 20px; }
    ...
    $TotalProfitPer=0;
        ...
        foreach($eq_values_data as $row)
        {
            ...
            if($footer) {
                $htmlEqVal .= '<tr nobr="true">
                        ...
                        if($brokerID=='0004' || $brokerID=='0009' || $brokerID=='0174')
                           {
                               $htmlEqVal .= '<td class="dataTotal noWrap border"></td><td class="dataTotal noWrap border"></td>';
                           }
                        $htmlEqVal .= '</tr>';
                        $htmlEqVal .= '
            ...
            if($header) {
                $htmlEqVal .= '<tr nobr="true">
                        ...
                        <tr nobr="true" class="head-row">
                            <th width="120">Scrip Name</th>
                            <th width="70">Quantity</th>';
                               if($IsAPCValueAvailable>0)
            {
            $htmlEqVal .= '                <th width="70">Purchase Price</th>
                            <th width="90">Purchase value</th>';
            }
                            $htmlEqVal .= '<th width="120">Market Price</th>
                            <th width="70">Market Value</th>';
        if($IsAPCValueAvailable>0)
            {
            $htmlEqVal .= '<th width="90">Profit</th>
                                <th width="70">% gain</th>';
            }
                                               
                            
                           $htmlEqVal .= ' <th width="70">% to Portfolio</th>';
                           if($brokerID=='0004' || $brokerID=='0009' || $brokerID=='0174')
                           {
                            $htmlEqVal .= '<th width="120">Industry</th><th width="90">Cap</th>';
                           }
                        $htmlEqVal .='</tr>
                    </tbody>
                    <tbody>';
            ...
            $totalPSum= round($totalPSum + $row->purchase_value);
                ...
                $htmlEqVal .= '<tr nobr="true">
                        ...
                        <td class="border normal">'.$row->quantity.'</td>';
                          ...
                          if($IsAPCValueAvailable>0)
            {                        
                   $htmlEqVal .= '     <td class="border normal amount">'.$row->apc.'</td>
                           ...
                           if($IsAPCValueAvailable>0)
            {
                   ...
                   $profit=($row->value-$row->purchase_value);
                        ...
                        if(!empty($profit) && !empty($pval)) {
                            ...
                            if($profitPer>0)
                            {
                                $htmlEqVal .= '<td class="border normal">'.$profitPer.'%1</td>';
                            }
                            else
                            {
                                $htmlEqVal .= '<td class="border normal"> - '.abs($profitPer).'%1</td>';
                            }
        ...
        $htmlEqVal .= '<tr nobr="true">
                        ...
                        if(!empty($ccPercent)) {
                            ...
                            if($brokerID=='0004' || $brokerID=='0009' || $brokerID=='0174')
                           {  $htmlEqVal .= '<td class="border normal"></td><td class="border normal"></td>';
        ...
        $htmlEqVal .= '<tr nobr="true">
                             ...
                             if($IsAPCValueAvailable>0)
                    {
                      if(!empty($totalPProfit)) {
                        ...
                        $htmlEqVal .= '<td class="dataTotal amount noWrap border"></td><td class="dataTotal amount noWrap border"></td>';
    ...
    <div id='wrap'>
        ...
        <div class="container">
            <form action="#" id="report_form" method="post" class="form-horizontal row-border">
                <div class="panel panel-midnightblue" style="margin-left: 15%; margin-right: 15%; width: 70%;">
