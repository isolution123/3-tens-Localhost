    ...
    if($mf_rep_data)
    {
        ...
        if($schemeFoot)
            ...
            if($familyFoot)
            {
                ...
                $html .= '<tr>
                        <td colspan="17" class="no-border info">
                          Weighted Avg. CAGR: '.$gTotalCagr.' | Weighted Avg. Abs Return: '.$gTotal_abs.' | Notional (Gain/Loss).'.$this->common_lib->moneyFormatIndia($gTotal - $gTotalPurAmt).'
                        </td>
                    </tr>
            ...
            //set purchase amount
                $purAmt=$mf_rep_data[$i]->purchase_amount;
                $divAmt=$mf_rep_data[$i]->div_amount;
