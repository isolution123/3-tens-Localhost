...
if(empty($mf_rep_data))
{
  ...
  $title = '<div class="title row">Type wise Mutual Fund Portfolio of '.$familyName.' Family</div><br/>';
 ...
 function hasPropertyValue($objects, $property, $value,$property1, $value1){
  ...
  {
      if(!in_array($row->client_name, $clientlist) )
      {
        array_push($clientlist, $row->client_name);
      }
  }
  ...
  $i=0;
    foreach($clientlist as $client)
    {
        ...
        foreach ($scheme_group_list as $scheme_group_type_name) {
            ...
            if(!hasPropertyValue($mf_rep_data,'client_name',$client,'Scheme_Group_TypeName',$scheme_group_type_name))
            {
                continue;
            }
          ...
          foreach($mf_rep_data as $row)
          {
              ...
              if($schemeFoot) 
                ...
                {
                    $html .= '<tr nobr="true">
                        <td class="no-border client-name2" colspan="4"><h4>'.$client.'</h4></td>
                    </tr>';
                    $newClient = false;
                }
               if($header)
               {
                   if($scheme_group_type_show)
                    {
                        ...
                        $html .= '<th colspan="7" style="font-size:18px; border-color: white;" width="500px">'.$scheme_group_type_name.'</th></tr>';
                 ...
                 if($row->client_name == $client  && $row->Scheme_Group_TypeName==$scheme_group_type_name)
                {
              if($content)
                {
