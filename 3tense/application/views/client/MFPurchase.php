        ...
        <div class="col-12 col-sm-12 col-md-12 col-lg-12 text-center p-0 mt-3 mb-2">
            <div class="card px-0 pt-4 pb-0 mt-3 mb-3">
                ...
                <div class="panel-heading">
                    <h4  >Lumpsum Purchase <a class="btn btn-default" href="<?php echo site_url('client/Purchase')?>"  style='float:right;margin-top:-7px;'>Back</a></h4>
                </div>
                  </div>
                <div class="row">
                                ...
                                <div class="form-card">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12" style="color:black">
                                           <B> Note :</B> NAV will be applicable as per funds realization subject to RTA/AMCs T&C.
                                        </div>
                                    </div>
                                </div> 
                                <input type="button" name="previous" class="previous action-button-previous" value="Previous" /> 
                            ...
                            <fieldset>
                                 <div class="form-card">
                                    <h2 class="fs-title">Make Payment</h2> 
                                     <div class="row">
                                        <input type="hidden" id="trancationId"/>
                                        <div class="form-group col-md-6 bank_id">
                                        ...
                                        <div class="form-group col-md-6 account_Id">    
                                ...
                                <input type="button" name="previous" class="previous action-button-previous" value="Previous" /> 
                                <input type="button" name="MakePayment" class="action-button MakePayment" style="width:132px" value="Make Payment" /> 
                            </fieldset>
                        </form>
                    </div>
                </div>
...
<script type="application/javascript">
     var banklist=[
                ...
                {id:"SWB",name :'SARASWAT COOPERATIVE BANK LIMITED',isdirect:0},
    ...
    $(function(){
        ...
        $("#schemename").select2({
            ...
            tags: [],
            ajax: {
                url: "<?php echo site_url('client/PurchaseMutualFund/get_bsc_scheme_list'); ?>",
                dataType: 'json',
                type: "POST",
                quietMillis: 50,
                data: function (term) {
        ...
        $(".next").click(function(){
            ...
            else if($(this).hasClass('nextscheme'))
            {
                /* if(!$('#schemetype').val())
                {
                    bootbox.alert("Please select Scheme Type.");
            ...
            else if($(this).hasClass('nextverifyscheme'))
            {
                ...
                {
                    bootbox.alert("Please Verify Terms & Condition.");
                }
                else
                {
                    /*var c_data=JSON.parse(readCookie('s_data'));   
                    if(c_data.trancationId>0)
                    {
                        isValidClick=true;
                    }
                    else
        ...
        function bindAccountDetail(obj)
        {
            ...
            if(obj)
            {
                ...
                str+='<br/><br/><table class="table table-striped table-bordered table-full-width dataTable" width="100%"><tr><th>Account Type</th><th>Bank Name</th><th>Branch</th><th>Account No</th></tr>';
                if(obj.ACCNO1)
                {
                    str+="<tr><td>"+obj.ACCTYPE1+"</td><td>"+obj.BANKNAME1+ "</td><td>"+obj.BANKBRANCH1+ "</td><td>"+obj.ACCNO1+ "</td></tr>";
                }
                if(obj.ACCNO2)
                {
                    str+="<tr><td>"+obj.ACCTYPE2+"</td><td>"+obj.BANKNAME2+ "</td><td>"+obj.BANKBRANCH2+ "</td><td>"+obj.ACCNO2+ "</td></tr>";
                }
                if(obj.ACCNO3)
                {
                    str+="<tr><td>"+obj.ACCTYPE3+"</td><td>"+obj.BANKNAME3+ "</td><td>"+obj.BANKBRANCH3+ "</td><td>"+obj.ACCNO3+ "</td></tr>";
                }
                if(obj.ACCNO4)
                {
                    str+="<tr><td>"+obj.ACCTYPE4+"</td><td>"+obj.BANKNAME4+ "</td><td>"+obj.BANKBRANCH4+ "</td><td>"+obj.ACCNO4+ "</td></tr>";
                }
                if(obj.ACCNO5)
                {
                    str+="<tr><td>"+obj.ACCTYPE5+"</td><td>"+obj.BANKNAME5+ "</td><td>"+obj.BANKBRANCH5+ "</td><td>"+obj.ACCNO5+ "</td></tr>";
                }
                str+='</table>';
        ...
        function bindAMC(schemetype)
        {
            ...
            $.ajax({
                url: "<?php echo site_url('client/PurchaseMutualFund/get_bsc_amc_list'); ?>",
                dataType: 'html',
                type: "POST",
        ...
        function set_cookie() {
            ...
            createCookie('s_data',JSON.stringify(s_data),1);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var tblString='<tr>'+
                        ...
                        '<td><input class="form-control isnumeric" min="'+parseInt(data.MinimumPurchaseAmount)+'" style="padding: 0px 10px 0px 10px;width:95%;border: 3px solid #9e9e9e;height:50px" id="amount" name="amount" type="number" placeholder="Amount" >'+
                        '<span><b>Min Value:</b> <span id="schememinvalue">'+parseInt(data.MinimumPurchaseAmount)+'</span></span>'+
        ...
        function placeorder()
       {
                ...
                { 
                    if(data['Status'])
                    {
                        if(data['Message'].account_list)
                        {
                            ...
                            {
                                var obj=banklist.filter(function(obj){return (obj.name.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME1.toLocaleLowerCase())>-1 )
                                });
                                 if(obj.length>0)
                                {
                                    for(var i=0;i<obj.length;i++)
                                    {
                                       banklistobj.push(obj[i]); 
                                    }
                                }
                            ...
                            {
                                var obj=banklist.filter(function(obj){return obj.name.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME2.toLocaleLowerCase())>-1});
                                if(obj.length>0)
                                {
                                    for(var i=0;i<obj.length;i++)
                                    {
                            ...
                            {
                                var obj=banklist.filter(function(obj){return obj.name.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME3.toLocaleLowerCase())>-1});
                               if(obj.length>0)
                                {
                                    for(var i=0;i<obj.length;i++)
                                    {
                            ...
                            if(data['Message'].account_list[0].BANKNAME4!="")
                            {
                                var obj=banklist.filter(function(obj){return obj.name.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME4.toLocaleLowerCase())>-1});
                                if(obj.length>0)
                                {
                                    for(var i=0;i<obj.length;i++)
                                    {
                            ...
                            {
                               var obj=banklist.filter(function(obj){return obj.name.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME5.toLocaleLowerCase())>-1});
                                if(obj.length>0)
                                {
                                    for(var i=0;i<obj.length;i++)
                                    {
                            ...
                            if(banklistobj.length>0)
                            {
                                ...
                                {
                                    $('#bank_id').append($("<option></option>").attr("value", banklistobj[i].id).attr("bankname", data['Message'].account_list[0].BANKNAME1).attr("mode", banklistobj[i].isdirect).text(banklistobj[i].name)); 
                                }
                            }
                        }
                    }
        ...
        $("#bank_id").change(function(){
                      ...
                      if(data['Status'])
                      {
                         if(data['Message'].account_list)
                         {
                            var bankname=$("#bank_id :selected").text();
                             ...
                             if(bankname.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME1.toLocaleLowerCase())>-1 && data['Message'].account_list[0].ACCNO1!='')
                             {
                                $('#account_number').append($("<option></option>").attr("value", data['Message'].account_list[0].ACCNO1).attr("ifsccode", data['Message'].account_list[0].NEFTIFSCCODE1).text(data['Message'].account_list[0].ACCNO1)); 
                             }
                             if(bankname.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME2.toLocaleLowerCase())>-1 && data['Message'].account_list[0].ACCNO2!='')
                             {
                                $('#account_number').append($("<option></option>").attr("value", data['Message'].account_list[0].ACCNO2).attr("ifsccode", data['Message'].account_list[0].NEFTIFSCCODE2).text(data['Message'].account_list[0].ACCNO2)); 
                             }
                             if(bankname.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME3.toLocaleLowerCase())>-1 && data['Message'].account_list[0].ACCNO3!='')
                             {
                                $('#account_number').append($("<option></option>").attr("value", data['Message'].account_list[0].ACCNO3).attr("ifsccode", data['Message'].account_list[0].NEFTIFSCCODE3).text(data['Message'].account_list[0].ACCNO3)); 
                             }
                             if(bankname.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME4.toLocaleLowerCase())>-1 && data['Message'].account_list[0].ACCNO4!='')
                             {
                                $('#account_number').append($("<option></option>").attr("value", data['Message'].account_list[0].ACCNO4).attr("ifsccode", data['Message'].account_list[0].NEFTIFSCCODE4).text(data['Message'].account_list[0].ACCNO4)); 
                             }
                             if(bankname.toLocaleLowerCase().indexOf(data['Message'].account_list[0].BANKNAME5.toLocaleLowerCase())>-1 && data['Message'].account_list[0].ACCNO5!='')
                             {
                                $('#account_number').append($("<option></option>").attr("value", data['Message'].account_list[0].ACCNO5).attr("ifsccode", data['Message'].account_list[0].NEFTIFSCCODE5).text(data['Message'].account_list[0].ACCNO5)); 
                             }
                         }
                      }
                      else
            ...
            {
                bootbox.alert("Your trancation is failed. Please contact administrator");
            }
