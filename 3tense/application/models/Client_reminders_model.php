...
class Client_reminders_model extends CI_Model{
    ...
    function dash_reminder_list($conditions, $limit)
    {
        ...
        reminder_status, if(length(reminder_message) > 600, Concat(left(reminder_message, 600), "..."), reminder_message) reminder_message ,
        attachment_url,
    ...
    function client_reminder_list($conditions, $limit)
    {
        $this->db->select('reminder_id, reminder_type, client_id, client_name, DATE_FORMAT(reminder_date, "%d/%m/%Y") as reminder_date,
        reminder_status, if(length(reminder_message) > 600, Concat(left(reminder_message, 600), "..."), reminder_message) reminder_message ,
        concern_user, broker_id');
    ...
    {
        $this->db->select('reminder_id, reminder_type, client_name, DATE_FORMAT(reminder_date, "%d/%m/%Y") as reminder_date,
                            reminder_message, remark,title,attachment_url');
        $this->db->from('today_reminders');
        $this->db->where($conditions);
        $query = $this->db->get();
        return $query->row();
