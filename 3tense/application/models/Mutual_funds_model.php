...
class Mutual_funds_model extends CI_Model{
    ...
    orig_trxn_no, orig_trxn_type, adjustment_ref_number, adjustment_flag, mf.bank_id, b.bank_name, branch, account_number, cheque_number,mfs.prod_code,mf.added_on,mf.from_file");
    ...
    {   
        ...
        //$this->get_datatables_query();
        if($this->session->userdata('broker_id')=='0004' || $this->session->userdata('broker_id')=='0009' || $this->session->userdata('broker_id')=='0174')
        {
            $this->db->select(" transaction_id, mf.client_id, c.name as client_name, c.family_id, fam.name as family_name, scheme_name,
            mfs.scheme_type_id, mft.scheme_type,
            (case when mf.mutual_fund_sub_type!='' then mf.mutual_fund_sub_type else mf.mutual_fund_type end )as mutual_fund_type, 
            transaction_type, folio_number, Date_Format(purchase_date, '%d/%m/%Y') as purchase_date, quantity, nav, mf.amount, adjustment,balance_unit,
            orig_trxn_no, orig_trxn_type, adjustment_ref_number, adjustment_flag, mf.bank_id, b.bank_name, branch, account_number, cheque_number,mfs.prod_code,mf.from_file,mf.added_on");
        }
        else
        {
            $this->db->select(" transaction_id, mf.client_id, c.name as client_name, c.family_id, fam.name as family_name, scheme_name,
            mfs.scheme_type_id, mft.scheme_type,
            mf.mutual_fund_type as mutual_fund_type, 
            transaction_type, folio_number, Date_Format(purchase_date, '%d/%m/%Y') as purchase_date, quantity, nav, mf.amount, adjustment,balance_unit,
            orig_trxn_no, orig_trxn_type, adjustment_ref_number, adjustment_flag, mf.bank_id, b.bank_name, branch, account_number, cheque_number,mfs.prod_code,mf.from_file,mf.added_on");
        }
        $this->db->from($this->table);
    ...
    function get_mutual_funds_duplicate($mycondition, $condition, $order="", $limit="")
    {   
        ...
        $this->db->where('orig_trxn_no in ((select orig_trxn_no FROM mutual_fund_transactions  
where broker_id="'.$this->session->userdata('broker_id').'" and ignore_from_duplicate=0 
group by client_id,mutual_fund_scheme,mutual_fund_type,folio_number,purchase_date,orig_trxn_no
having count(transaction_id)>1))');
    ...
    function get_mutual_funds_valuation_extended($mycondition, $condition, $order="", $limit="") {
        ...
        $this->db->select("s.prod_code,f.name as family_name, c.name as client_name, t.client_id, s.scheme_name as mf_scheme_name, t.mutual_fund_scheme as scheme_id,
            t.folio_number, DATE_FORMAT(t.purchase_date,'%d/%m/%Y') as purchase_date, 
            (case when t.mutual_fund_sub_type!='' then t.mutual_fund_sub_type else t.mutual_fund_type end )as trn_type, 
            round(v.p_amount) as p_amount, t.nav as p_nav, v.c_nav, DATE_FORMAT(v.c_nav_date,'%d/%m/%Y') as c_nav_date, v.live_unit, round(v.div_r2) as div_r2, round(v.div_payout) as div_payout,
            round(v.div_amount) as div_amount, v.transaction_day, v.mf_abs, v.mf_cagr as cagr, st.scheme_type, round(v.c_nav * v.live_unit, 2) as current_value,t.from_file");    
        }
        else
        {
            $this->db->select("s.prod_code,f.name as family_name, c.name as client_name, t.client_id, s.scheme_name as mf_scheme_name, t.mutual_fund_scheme as scheme_id,
            t.folio_number, DATE_FORMAT(t.purchase_date,'%d/%m/%Y') as purchase_date, 
            t.mutual_fund_type as trn_type, 
            round(v.p_amount) as p_amount, t.nav as p_nav, v.c_nav, DATE_FORMAT(v.c_nav_date,'%d/%m/%Y') as c_nav_date, v.live_unit, round(v.div_r2) as div_r2, round(v.div_payout) as div_payout,
            round(v.div_amount) as div_amount, v.transaction_day, v.mf_abs, v.mf_cagr as cagr, st.scheme_type, round(v.c_nav * v.live_unit, 2) as current_value,t.from_file");
        }
    ...
    function update_right_trancation_flag($transaction_id, $broker_id)
    {
            
        $this->db->set('a.ignore_from_duplicate', 1);
        $this->db->where('a.transaction_id',  $transaction_id);
        $this->db->where('a.broker_id',  $broker_id);
       
        if(!( $this->db->update('mutual_fund_transactions as a'))) {
            $error = $this->db->error();
            return $error;
        } else {
            return $this->db->affected_rows();
    ...
    {
        $this->db->select('sh.scheme_history_id, sh.scheme_id, s.scheme_name, s.prod_code,sisin.isin,s.scheme_type_id, st.scheme_type, sh.current_nav,
        ...
        $this->db->join('mutual_fund_schemes_isin sisin','s.scheme_id = sisin.scheme_id','inner');
    ...
    function get_mf_schemes_list()
    {
        $this->db->select('s.scheme_id, s.scheme_name, s.prod_code,sisin.isin,s.scheme_type_id, st.scheme_type,s.market_cap');
        $this->db->from('mutual_fund_schemes s');
        $this->db->join('mutual_fund_schemes_isin sisin','s.scheme_id = sisin.scheme_id','inner');
        $this->db->join('mf_scheme_types st','st.scheme_type_id = s.scheme_type_id','left');
    ...
    function add_import_mutual_funds($mfData, $transID)
    {
       ...
       /* if($remData != null)
        ...
        if($result) {
        		 if ($this->session->userdata('broker_id')=='0180' || $this->session->userdata('broker_id')=='0004' || $this->session->userdata('broker_id')=='0009' )
        		 {
    ...
    function mf_calculate_live_units_delete_op_historical_c($brokerID, $reportDate,$clientID) {
        $query1 = $this->db->query("call sp_mf_calculate_live_units_delete_op_historical_c(?, ?,?)", array('brokerID'=>$brokerID, 'reportDate'=>$reportDate,'clientID'=>$clientID));
        if(!$query1) {
            return $this->db->error();
        } else {
            return true;
    ...
    function get_mutual_fund_family_summary_portfolio_new($type,$where)
    {
      $query="";
     
        $query = $this->db->query("call sp_mf_schemewise_summary_report_clientwise_new(?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_mutual_fund_family_summary_portfolio_historical($type,$where)
    {
     ...
     function get_mutual_fund_family_summary_portfolio_historical_new($type,$where)
    {
      
        $query = $this->db->query("call sp_mf_schemewise_summary_report_clientwise_historical_new(?,?,?,?)", $where);
    ...
    function get_mutual_fund_family_summary_typewise($type,$where)
    {
      ...
      function get_mutual_fund_family_summary_typewise_new($type,$where)
    {
      $query="";
     
        $query = $this->db->query("call sp_mf_schemewise_summary_report_typewise_new(?,?,?)", $where);
     ...
     function get_mutual_fund_family_summary_typewise_historical($type,$where)
    {
      ...
      function get_mutual_fund_family_summary_typewise_historical_new($type,$where)
    {
      $query="";
      
        $query = $this->db->query("call sp_mf_schemewise_summary_report_typewise_historical_new(?,?,?,?)", $where);
    ...
    function get_mutual_fund_family_detail_schemewise($type,$where)
    {
     ...
     function get_mutual_fund_family_detail_schemewise_new($type,$where)
    {
      $query="";
      
        $query = $this->db->query("call sp_mf_schemewise_detail_new(?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_mutual_fund_family_detail_schemewise_historical($type,$where)
    {
     ...
     function get_mutual_fund_family_detail_schemewise_historical_new($type,$where)
    {
      $query="";
      
        $query = $this->db->query("call sp_mf_schemewise_detail_historical_new(?,?,?,?)", $where);
    ...
    function get_mutual_fund_family_summary_schemewise_net_investment($type,$where)
    {
     ...
     function get_mutual_fund_family_summary_schemewise_net_investment_new($type,$where)
    {
      $query="";
      
        $query=$this->db->query("call sp_mf_schemewise_summary_net_invest_new(?,?,?)", $where);
    ...
    function get_mutual_fund_family_summary_schemewise_net_investment_historical_new($type,$where)
    {
      $query="";
      
        $query=$this->db->query("call sp_mf_schemewise_summary_net_invest_historical_new(?,?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_clientwise_summary_year_wise($type,$where)
    {
      $query="";
      //echo '<pre>';print_r($where);die;
     
        $query=$this->db->query("call sp_mf_clientwise_summary_year_wise(?,?,?)", $where);
    ...
    function get_clientwise_summary_new($type,$where)
    {
      $query="";
      //echo '<pre>';print_r($where);die;
     
        $query=$this->db->query("call sp_mf_clientwise_summary_new(?,?,?)", $where);
    ...
    function get_clientwise_summary_historical($type,$where)
    {
     ...
     function get_clientwise_summary_year_wise_historical_new($type,$where)
    {
        $query="";
        //echo '<pre>';print_r($where);die;
        $query = $this->db->query("call sp_mf_clientwise_summary_year_wise_historical_new(?,?,?,?)", $where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
     ...
     function get_clientwise_summary_historical_new($type,$where)
    {
        $query="";
        //echo '<pre>';print_r($where);die;
        $query = $this->db->query("call sp_mf_clientwise_summary_historical_new(?,?,?,?)", $where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function mf_comman_scheme_summary($type,$where)
    {
      ...
      if($type=='family')
      {
       ...
       function mf_comman_scheme_summary_new($type,$where)
    {
      $query="";
    
          $query=$this->db->query("call sp_mf_comman_scheme_summary_new(?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
     ...
     function mf_comman_cap_detail_1($type,$where)
    {
      $query="";
          $query=$this->db->query("call sp_mf_comman_cap_detail_1(?,?,?)", $where);
      ...
      function mf_comman_cap_detail_1_new($type,$where)
    {
      $query="";
          $query=$this->db->query("call sp_mf_comman_cap_detail_1_new(?,?,?)", $where);
    ...
    function mf_comman_scheme_summary_historical($type,$where)
    {
     ...
     function mf_comman_scheme_summary_historical_new($type,$where)
    {
      $query="";
      
          $query=$this->db->query("call sp_mf_comman_scheme_summary_historical_new(?,?,?,?)", $where);
    ...
    function mf_comman_cap_detail_1_historical($type,$where)
    {
      $query="";
      
          $query=$this->db->query("call sp_mf_comman_cap_detail_1_historical(?,?,?,?)", $where);
    ...
    function mf_comman_cap_detail_1_historical_new($type,$where)
    {
      $query="";
      
          $query=$this->db->query("call sp_mf_comman_cap_detail_1_historical_new(?,?,?,?)", $where);
    ...
    function mf_summary_detail_historical($type,$where)
    {
      $query="";
      
          $query=$this->db->query("call sp_mf_summary_detail_historical(?,?,?,?)", $where);
     ...
     function mf_summary_detail($type,$where)
    {
      $query="";
          $query=$this->db->query("call sp_mf_summary_detail(?,?,?)", $where);
      ...
      function mf_comman_cap_detail_test($type,$where)
    {
      $query="";
      
        $query = $this->db->query("call sp_mf_comman_cap_detail_2(?,?,?)",$where);
    ...
    function mf_comman_cap_detail_2_historical($type,$where)
    {
      $query="";
      
          $query=$this->db->query("call sp_mf_comman_cap_detail_2_historical(?,?,?,?)", $where);
    ...
    function get_mutual_funds_trancation_amount($type,$where)
    {
      
         $query="";
      
          $query=$this->db->query("call get_mutual_funds_trancation_amount(?,?,?,?)", $where);
     ...
     function get_mutual_funds_trancation_amount_new($type,$where)
    {
      
         $query="";
      
          $query=$this->db->query("call get_mutual_funds_trancation_amount_new(?,?,?,?)", $where);
    ...
    function  folio_wise_summary($type,$where)
    {
     ...
     function  folio_wise_summary_new($type,$where)
    {
        $query="";
       
            $query=$this->db->query("call sp_mf_folio_wise_new(?,?,?)", $where);
        if($query)
        {
              $res = $query->result();
              $query->next_result();
      ...
      function  folio_wise_summary_historical($type,$where)
    {
      ...
      function  folio_wise_summary_historical_new($type,$where)
    {
        $query="";
            $query = $this->db->query("call sp_mf_folio_wise_historical_new(?,?,?,?)",$where);
    ...
    function get_sip_report($type,$where)
    {
      ...
      function get_sip_report_new($type,$where)
    {
        $query="";
        $query = $this->db->query("call sp_mf_sip_new(?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_sip_report_historical($type,$where)
    {
     ...
     function get_sip_report_historical_new($type,$where)
    {
        $query="";
        $query = $this->db->query("call sp_mf_sip_historical_new(?,?,?,?)", $where);
          if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_mutual_fund_report($type, $where)
    {
     ...
     function get_mutual_fund_report_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("call sp_mf_report_family_new(?, ?, ?)", $where);
        }
        if($query) {
            //To execute multiple queries
            $res = $query->result();
    ...
    function get_mutual_fund_report_historical($type, $where)
    {
     ...
     function get_mutual_fund_report_historical_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            //$query = $this->db->query("call sp_mf_valuation_family(?, ?)", $where);
            $query = $this->db->query("call sp_mf_report_family_historical_new(?, ?, ?,?)", $where);
        }
        if($query) {
            //To execute multiple queries
            $res = $query->result();
    ...
    function get_net_investment($type, $where)
    {
      ...
      function get_net_investment_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("select calculateNetInvestmentFamily_new(?, ?, ?) as net_Investment", 
                $where);
        }
        $res = $query->result();
        return $res;
     ...
     function get_net_investment_historical($type, $where)
    {
     ...
     function get_net_investment_historical_new($type, $where)
    {
        $query = "";
         if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("select calculateNetInvestmentFamily_historical_new(?, ?, ?, ?) as net_Investment", $where);
        }
    ...
    function get_investment_summary_historical($type, $where)
    {
     ...
     function get_investment_summary_historical_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("call sp_get_investment_summary_family_historical_new(?, ?,?,?)", $where);
        }
        //To execute multiple queries
        $res = $query->result();
        // changes in system/database/drivers/mysqli/mysqli_result.php
    ...
    function get_investment_summary($type, $where)
    {
     ...
     function get_investment_summary_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("call sp_get_investment_summary_family_new(?, ?, ?)", $where);
        }
        //To execute multiple queries
        $res = $query->result();
        $query->next_result();
    ...
    function get_current_value_summary($type, $where)
    {
     ...
     function get_current_value_summary_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("call sp_get_current_value_summary_family_new(?, ?, ?)", $where);
        }
        //To execute multiple queries
        $res = $query->result();
        // changes in system/database/drivers/mysqli/mysqli_result.php
     ...
     function get_current_value_summary_historical($type, $where)
    {
     ...
     function get_current_value_summary_historical_new($type, $where)
    {
        $query = "";
        if($type == 'client')
        {
        ...
        {
            $query = $this->db->query("call sp_get_current_value_summary_family_historical_new(?, ?,?,?)", $where);
        }
        //To execute multiple queries
        $res = $query->result();
        // changes in system/database/drivers/mysqli/mysqli_result.php
    ...
    function mf_calculate_divp_delete_op_historical($condition)
    {
          $query = $this->db->query("call sp_mf_calculate_divp_delete_op_historical(?, ?)", $condition);
          if($query)
          {
              return true;
          } else {
    ...
    function mf_calculate_divr_delete_op_historical($condition)
    {
          $query = $this->db->query("call sp_mf_calculate_divr_delete_op_historical(?, ?)", $condition);
          if($query)
          {
              return true;
          } else {
    ...
    function mf_update_c_nav_delete_op_historical($condition)
    {
          $query = $this->db->query("call sp_mf_update_c_nav_delete_op_historical(?,?,?)", $condition);
          if($query)
          {
              return true;
          } else {
    ...
    public function insert_nav_1($data)
    {
         if(!$this->db->insert('mf_schemes_histories',$data)) {
            return $this->db->error();
        } else {
            return $this->db->insert_id();
        }
    ...
    function  get_aum_report($where)
    {
     ...
     function  get_aum_report_for_chart($where)
    {
        $query="";
        $query = $this->db->query("call sp_mf_aum_report_for_chart(?)",$where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
      ...
      function  detail_aum_report_from_file($where)
    {
        $query="";
        $query = $this->db->query("call mf_detail_aum_report_from_file(?)",$where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
     ...
     function get_all_summary($where)
    {
        $query="";
        //echo '<pre>';print_r($where);die;
        $query = $this->db->query("call sp_mf_all_summary(?)", $where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    function get_family_wise_summary_aum($where)
    {
     
        $query="";
        //echo '<pre>';print_r($where);die;
        $query = $this->db->query("call sp_family_wise_summary_aum_report(?)", $where);
        if($query)
           {
              $res = $query->result();
              $query->next_result();
    ...
    //Dipak 11 may
      function capital_gain($type,$where)
      {
        ...
        function capital_gain_new($type,$where)
      {
        // print_r($where);
          $query="";
          $query = $this->db->query("call sp_mf_capitalgain_report_new(?,?,?,?,?)", $where);
            if($query)
             {
                $res = $query->result();
                $query->next_result();
    ...
    function follio_master($type,$where)
    {
      ...
      function follio_master_new($type,$where)
    {
      $query="";
      $query=$this->db->query('call sp_mf_folio_master_new(?,?,?)',$where);
      if($query)
      {
        $res=$query->result();
        $query->next_result();
   ...
   function update_curr_nav()
    {
        ...
        $query1 = $this->db->query("TRUNCATE TABLE `mf_schemes_current_nav`");
        $query2 = $this->db->query("insert into mf_schemes_current_nav(`scheme_id`,`current_nav`,`scheme_date`)
                                    select mf_schemes_histories.`scheme_id`, mf_schemes_histories.`current_nav`, mf_schemes_histories.`scheme_date`
                                    from mf_schemes_histories
                                    join 
                                    (
                                        select scheme_id, max(scheme_date) as max_dt 
                                        from mf_schemes_histories
                                        group by scheme_id
                                    ) t
                                    on mf_schemes_histories.scheme_id= t.scheme_id and mf_schemes_histories.scheme_date = t.max_dt");
