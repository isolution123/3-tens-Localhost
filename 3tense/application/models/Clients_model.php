...
class Clients_model extends CI_Model {
    var $table = 'clients';
    var $column = array('client_id, clients.name as c_name,passport_no, clients.family_id, families.name as f_name, relation_HOF, client_type, client_type_name, email_id,date_format(dob,"%d/%m/%Y") as dob, pan_no, mobile, username, date_format(date_of_comm,"%d/%m/%Y") as date_of_comm, client_category, (case when clients.status = 1 then "Active" else "Inactive" end) as status,app_access');
   ...
   function count_client($brokerID)
    ...
    function get_scheme_by_scheme_name($scheme_name)
    {
                $this->db->select('*');
    ...
    function add_client_import($data)
    {
       if($this->db->insert('clients', $data)) {
     ...
     function get_client_family_by_inv_name($condition)
   {
    ...
    {
        $this->db->select('f.family_id, f.name,broker_id');
    ...
    function get_clients_broker_dropdown_wise($condition)
    {
        //$this->db->select('client_id, c.name');
        $this->db->select('client_id, c.name,date_format(c.dob,"%d/%m/%Y")as dob2,c.pan_no'); //Pallavi - 2017-06-12
        $this->db->from('clients as c');
        $this->db->join('families as fam', 'fam.family_id = c.family_id', 'left');
        $this->db->where($condition);
        //Pallavi - 2017-06-12
        $this->db->where('c.merge_ref_id',null); //added by Pallavi for client merging.
        $this->db->order_by('c.name','asc');
...
$res1=$query1->result_array();
  ...
  //   echo $this->db->last_query();
    ...
    function setDeviceDetails($client_id,$data)
    {
        $this->db->where('client_id', $client_id);
        return $this->db->update('clients', $data);
    }
    
    function SaveDeviceDetail($device_token,$data)
    {
        $flag=0;
        $this->db->select('Id,client_id');
        $this->db->from('client_device_detail');
        ...
        if($query) {
            ...
            if(!empty($result)) {
                if(!empty($result[0]['Id'])) {
                    ...
                    $this->db->update('client_device_detail', $data);
                    $flag=1;
                } 
        ...
        if($flag==0)
        {
            if($this->db->insert('client_device_detail', $data)) {
                return 1;
            } else {
                return $this->db->error();
            }
    ...
    function get_client_category_dropdown()
    {
        $this->db->select('DISTINCT(client_category)');
        $this->db->from('clients');
        $this->db->where("client_category IS NOT NULL AND client_category!='' AND device_token IS NOT NULL");
        $this->db->order_by("client_category", "asc");
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_clients_with_device_token()
    {
        $this->db->distinct();
        $this->db->select('c.client_id,c.name,c.device_token,c.device_type,c.device_os,c.user_id, broker_id,client_category');
        $this->db->from('clients c');
        $this->db->join('families f','f.family_id = c.family_id','inner');
        $this->db->where('c.device_token IS NOT NULL AND c.status=1');
        $this->db->order_by("c.name", "asc");
        $query = $this->db->get();
        //echo '<pre>';print_r($this->db->error());die;
        return $query->result();
    }
    function get_clients_with_device_token_1()
    {
        $this->db->select('c.client_id,c.name,t.device_token,t.device_type,t.device_os,c.user_id, broker_id,client_category');
        $this->db->from('clients c');
        $this->db->join('families f','f.family_id = c.family_id','inner');
        $this->db->join('client_device_detail t','t.client_id = c.client_id','inner');
        $this->db->where('t.device_token IS NOT NULL AND c.status=1');
        $this->db->order_by("c.name", "asc");
        $query = $this->db->get();
        //echo '<pre>';print_r($this->db->error());die;
        return $query->result();
