...
class Fixed_deposits_model extends CI_Model{
    ...
    nominee as nominee, fdt.status, adv_id, adviser_name, adjustment, maturity_payout_id, adjustment_flag,
...
function get_fixed_deposit_maturity($condition)
        {
          $this->db->select(" c.name as client_name, fd_comp_name, ref_number,Date_Format(maturity_date, '%d/%m/%Y') as maturity_date,maturity_amount");
    ...
    public function getNominee($condition)
    {
      $this->db->select('c.client_id');
      $this->db->from('clients c');
      $this->db->join('families f','c.family_id = f.family_id', 'inner');
      $this->db->where($condition);
      $query = $this->db->get();
      if(!$query)
          return false;
    ...
    {
      $this->db->select_max('fd_transaction_id');
      $this->db->from('fddemo');
      $query = $this->db->get();
      if(!$query)
          return false;
    ...
    public function get_fd_inv_id($condition)
    {
      $this->db->select('fd_inv_id');
      $this->db->from('fd_investment_types');
      $this->db->where($condition);
      $query = $this->db->get();
      if(!$query)
          return false;
    ...
    public function get_fd_comp_id($condition)
    {
      $this->db->select('fd_comp_id');
      $this->db->from('fd_companies');
      $this->db->where($condition);
      $query = $this->db->get();
      if(!$query)
          return false;
    ...
    public function get_adviser_id($condition)
    {
      $this->db->select('adviser_id');
    ...
    public function get_bank_id($condition)
    {
    ...
    public function confirm_ac_no($condition)
    {
    ...
    public function get_payout_id($condition)
    {
      $this->db->select('payout_mode_id');
    ...
    private function get_datatables_query()
    {
        ...
        $this->db->join('clients as c', 'fdt.client_id = c.client_id', 'inner');
        $this->db->join('families as fam', 'fam.family_id = c.family_id', 'inner');
        //$this->db->join('clients as nom', 'fdt.nominee = nom.client_id', 'left');
        ...
        $this->db->join('fd_payout_modes as fpm', 'fdt.maturity_payout_id = fpm.payout_mode_id', 'left');
...
function get_nominee_name($nominee_id)
{
  $this->db->select('name');
  $this->db->from('clients');
  $this->db->where($nominee_id);
  $query = $this->db->get();
  if(!$query)
  {
    return false;
