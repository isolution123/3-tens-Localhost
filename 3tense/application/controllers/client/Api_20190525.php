...
class Api extends CI_Controller
{
    ...
    private function getReminders($objRequest)
    {
        ...
        $date->modify('+15 day');
        $condition = 'reminder_date between "'.$date2->format('Y-m-d').'" and "'.$date->format('Y-m-d').'" AND reminder_type != "Client" AND client_id = "'.$objRequest->client_id.'" AND client_view = 0' ;
        $rem_data = $this->rem->dash_reminder_list($condition, 50);
        return array('code'=>1,'reminders'=>$rem_data,'reminder_count'=>count($rem_data));
    }
    ...
    private function getSummary($objRequest)
    {
        ...
        if(@$objRequest->hof && $client->head_of_family == '1')
        {
            ...
            //echo '<pre>';print_r($client_list);die;
            $data['members'] = array();
    ...
    private function getRealEstateReport($objRequest)
    {
        ...
        if(count($re_rep) > 0)
        {
            ...
            foreach($re_rep as $row)
            {
                ...
                if(!empty($rent)) {
                    $row->rent_amount = $this->common_lib->moneyFormatIndiaClient(round($rent_amount));
                } else {
                   $row->rent_amount = '';
                }
                if(array_key_exists($row->client_name, $client_data))
    ...
    private function getCashflowReport($objRequest)
    {
        ...
        if(!empty($cash_flow_rep))
        {
            ...
            foreach ($cash_flow_rep as $row)
            {
                ...
                if(!empty($life_cover))
                {
                    $row->Life_Cover = $this->common_lib->moneyFormatIndiaClient(round($Life_Cover),2);
                }
