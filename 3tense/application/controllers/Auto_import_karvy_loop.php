      ...
      $emails = imap_search($inbox, 'SUBJECT "Subscribed Master AUM/Investor Master Details Report for Reference No" ');
       // echo "<pre>";print_r($emails);exit;
          ...
          //  echo " Mails for today";
             ...
             if(isset($toaday_karvy_mail) && !empty($toaday_karvy_mail)){
...
//echo $final_link;
            if(strpos( $final_link,'/karvy/Subreports/')){
              ...
              //  $problem="false";
                $problem_message[]="file link is not available in the mail with Broker:".$broker_name."And date:".$comp_today_date;
            }
        ...
        // exit;
         ...
         $local = "downloads/Folio_master_list/karvy_folder/"; //local folder path
          ...
          if(!$filepath) {
              ...
              $problem_message[]=$bc_date.' - Could not download client folio master file with Broker:'.$broker_name."And date:".$comp_today_date;
             ...
             //below code added by Salmaan - 2017-11-02
              ...
              if($brokerName==='samor108@gmail.com'){
                  //echo "K k";
                        ...
                        if(!$pwd) {
                            $pwd = '';
                            echo 'No user found for mailback ID';
                            continue;
                        }
         ...
         $unzip = $this->extract_archive($filepath,$location,$pwd);
          if(!$unzip) {
              echo $bc_date.' - Could not open/extract zip file. <br/>';
                $problem_message[]=$bc_date.' - Could not open/extract zip file for the request id:'."with Broker:".$broker_name."And date:".$comp_today_date;
              //$bc_date = $this->get_yesterday($bc_date);
              continue;
          } else {
            ...
            $bc_url = $link;
             //get the date for which you want bhav_copy - start with todays date
                 ...
                 //now download the bhav copy from BSE website
                  $local = "downloads/MF_Transaction_list/karvy_folder/"; //local folder path
                   ...
                   if(!$filepath) {
                       ...
                       $problem_message[]=$bc_date.' - Could not download client folio master file with File:'.$bc_filename."And date:".$comp_today_date;
                     $bc_date = $this->get_yesterday($bc_date);
                       continue;
                   }
               ...
               //   $unzip=true;//comment this line on server and uncomment below line on server
                   ...
                   if(!$unzip) {
                       echo $bc_date.' - Could not open/extract zip file. <br/>';
                         $problem_message[]=$bc_date.' - Could not open/extract zip file for file:'.$bc_filename."And date:".$comp_today_date;;
                       //$bc_date = $this->get_yesterday($bc_date);
                       continue;
                   } else {
                     //scan the directory for today's cams folio master dbf file
...
//Karvy SIP import
 ...
 //echo "<pre>";print_r($semails);exit;
  ...
  if(isset($s_today_mail) && !empty($s_today_mail)){
        ...
        //    echo $bc_url;
          // exit;
           ...
           $local = "downloads/SIP_files_list/karvy_folder/"; //local folder path
            ...
            if(!$filepath) {
                echo $bc_date.' - Could not download bhav copy. <br/>';
                $sproblem_message[]=$bc_date.' - Could not download client folio master file with Broker:'.$broker_name."And date:".$comp_today_date;
              $bc_date = $this->get_yesterday($bc_date);
          ...
          //  $unzip=true;//comment this line on server and uncomment below line on server
           ...
           $unzip = $this->extract_archive($filepath,$location,$pwd2);
            if(!$unzip) {
                echo $bc_date.' - Could not open/extract zip file. <br/>';
                  $sproblem_message[]=$bc_date.' - Could not open/extract zip file for the request id:'."with Broker:".$broker_name."And date:".$comp_today_date;;
                //$bc_date = $this->get_yesterday($bc_date);
                continue;
            } else {
              //scan the directory for today's cams folio master dbf file
...
function clients_import($flag,$file,$passfileType)
{
  //echo"hellllllllllllllooooooooooooooo";//exit;
    ...
    $brokerID = $this->session->userdata('brokerID'); //added by Salmaan - 2017-11-02
...
if($fileType_info=="application/excel"||$fileType_info=="application/vnd.ms-excel"||$fileType_info=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
      ...
      //      echo"<pre>"; print_r($dataColumns);exit;//check-column-headings
                            ...
                            if($rta_list=='karvy_excel'){  // for CAMS RTA
                              unset($bankBranch,$bank_address_road,$bank_address_area,$bank_address_city);
                               ...
                               if(!empty($findkey=array_search('Broker Code',$dataColumns)))
                                 {
                                   $cell=$rows[$findkey];
                                     ...
                                     $arn=$broker_code;
                                     //commented below and added code by Salmaan - 2017-11-02
                                              ...
                                              $mfMessage = "Pan Number cannot be empty";*/
                                      ...
                                      else if(strtoupper($dataColumns[$countCell]) === 'ARN' || strtoupper($dataColumns[$countCell]) === 'TD_AGENT' || strtoupper($dataColumns[$countCell]) === 'AGENT CODE' || strtoupper($dataColumns[$countCell]) === 'BROKCODE' || strtoupper($dataColumns[$countCell]) === 'BROK_CODE')
                                                                {
                                                                    if($cell || $cell != ''){
                                                                      ...
                                                                      //commented by Salmaan - 2017-11-02
                                                                      /*$user=$this->Clients_model->get_userdata($arn);
                                                                    //  echo"<pre>";print_r($user);exit;
                                                                      if(count($user)>0){
                                                                        if($user[0]['status']==1 && $user[0]['user_type']=='broker'){
