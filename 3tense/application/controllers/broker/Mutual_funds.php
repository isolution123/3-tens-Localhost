...
class Mutual_funds extends CI_Controller{
    ...
    function index()
    {
      ...
      function MFDuplicateTrancation()
    {
        //data to pass to header view like page title, css, js
        $header['title']='Mutual Funds Duplicate Trancation';
        $header['css'] = array(
            'assets/users/plugins/datatables/css/jquery.dataTables.min.css',
            'assets/users/plugins/pines-notify/jquery.pnotify.default.css',
            'assets/users/plugins/form-daterangepicker/daterangepicker-bs3.css'
        ...
        //load views
        $this->load->view('broker/common/header', $header);
        $this->load->view('broker/mutual_fund/mf_duplicate_list');
        $this->load->view('broker/common/notif');
        $this->load->view('broker/common/footer');
    }
    ...
    public function ajax_list()
    {
        ...
        foreach($list['rResult'] as $mf)
        {
            ...
            $date=date_create($mf->added_on);
            $row['from_file'] = $mf->from_file;
             
            $row['added_on'] = date_format($date,"d/m/Y");
            $row['bank_name'] = $mf->bank_name;
            $row['account_number'] = $mf->account_number;
            $row['cheque_number'] = $mf->cheque_number;
            ...
            if($permissions=="3")
            {
                ...
                <i class="fa fa-pencil"></i></a>
                <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Delete"
                onclick="delete_mf('."'".$mf->transaction_id."'".','."'".$mf->client_id."'".')">
                <i class="fa fa-trash-o"></i></a>';
            }
            else if($permissions=="2")
            {
    ...
    public function ajax_duplicate_list()
    {
        $brokerID = $this->session->userdata('broker_id');
        //$list = $this->ins->get_insurances(array('ins.broker_id' => $brokerID));
        ...
        if($_GET['from']!='' || $_GET['to']!='')
        {
            ...
            */
            ...
            $list = $this->mf->get_mutual_funds_duplicate("mf.broker_id='".$brokerID."' ".$myWhere, $sWhere, $sOrder, $sLimit );
            //print_r($list);
        }
        else
        {
            
            $list = $this->mf->get_mutual_funds_duplicate("mf.broker_id='".$brokerID."'", $sWhere, $sOrder, $sLimit);
        ...
        foreach($list['rResult'] as $mf)
        {
            ...
            if($permissions=="3")
            {
                ...
                onclick="delete_mf('."'".$mf->transaction_id."'".','."'".$mf->client_id."'".')">
                <i class="fa fa-trash-o"></i></a>
                <a class="btn btn-sm btn-success" href="javascript:void(0)" title="mark as correct trancation"
                onclick="ok_mf('."'".$mf->transaction_id."'".')">
                <i class="fa fa fa-check-square-o"></i></a>';
            }
    ...
    function delete_mf()
    {
        ...
        try
        {
           ...
           $this->auto_mf_valuation_delete_op_historical($brokerID,date("Y-m-d"),$client_id);
            //if there is any error
            if(isset($status['code']))     
            {
                throw new Custom_exception();
    ...
    function update_right_trancation_flag()
    {
        $brokerID = $this->session->userdata('broker_id');
        $mf_id = $this->input->post('mf_id');
        ...
        try
        {
            $status = $this->mf->update_right_trancation_flag($mf_id,$brokerID);
            //if there is any error
            if(isset($status['code']))
            {
                throw new Custom_exception();
            ...
            $success = array(
                'status' => 'success',
                'title'=> 'Mutual Fund updated!',
                'text' => 'Mutual Fund Details for updated successfully'
            );
        }
        catch(Custom_exception $e)
        {
    ...
    function mf_import()
    {
        ...
        if (isset($_POST["Import"]))
        {
            ...
            if (isset($_FILES["import_mf"]))
            {
                ...
                //else if ($brokerID=='0004' || $brokerID=='0009' || $brokerID=='0174' && )
                else
                {
                    ...
                    if(isset($_POST["file_type"]))
                    {
                        ...
                        $from_file=$_POST["file_type"];
                    }
                    
                    $folioNum = ""; $clientName = ""; $panNum = ""; $mfType = "";$mfSubType = "";$SIPTRXNNO=""; $purDate = ""; $unit = ""; $schemeName = ""; $prodCode = "";
                        $nav = ""; $adj = ""; $clientId = ""; $familyId = ""; $adjRefNum = ""; $balUnit = ""; $amount = ""; $dpo = 0; $schemeId="";
                        $adj_flag = 0; $tempMFType = ""; 
                    ...
                    if(pathinfo($_FILES["import_mf"]["name"], PATHINFO_EXTENSION)=='dbf' || pathinfo($_FILES["import_mf"]["name"], PATHINFO_EXTENSION)=='DBF')
                    {
                    ...
                    foreach($excelData as $rows)
                    {
                        ...
                        foreach($rows as $cell)
                        {
                            ...
                            if($countRow == 0)
                            {
                                ...
                                if(strtoupper($cell) == 'SR NO' || strtoupper($cell) == 'TRXNNO' || strtoupper($cell) == 'TD_TRNO' || strtoupper($cell) == 'TRXN_NO' ||
									...
									|| strtoupper($cell) == 'TRXN MODE' || strtoupper($cell) == 'TRXNMODE'|| strtoupper($cell) == 'SIPTRXNNO' )
                                {
                                    $dataColumns[$countCell] = $cell;
                                    $countCell++;
                                    $uploadedStatus = 2;
                                ...
                                if($insertRow)
                                {
                                        ...
                                        strtoupper($dataColumns[$countCell]) == 'IT_PAN_NO1')
                                    {
                                        ...
                                        if($cell || $cell != '')
                                        {
                                            ...
                                            {
                                                $insertRow = false;
                                                $mfMessage = "Pan No. ".$panNum ." = ". $brokerID." Doesn't exists for any client1";
                                            }
                                            else
                                            {
                                    ...
                                    else if(strtoupper($dataColumns[$countCell]) =='SIPTRXNNO')
                                    {
                                        if($cell || $cell != '')
                                        {
                                            $SIPTRXNNO=$cell;
                                            if($mfSubType=='' &&   (strpos($tempMFType,"P") === 0))   
                                            {
                                                $mfSubType='SIP';
                                        ...
                                        strtoupper($dataColumns[$countCell]) == 'TRANSACTION TYPE' || strtoupper($dataColumns[$countCell]) == 'TRXN_TYPE')
                                    {
                                        if($cell || $cell != '')
                                        {
                                            ...
                                            if($mfType == "DIV" || $mfType == "DR" || $mfType == "DIR" || $mfType == "BNS" || $mfType == "BNSR" || $mfType == "DIRR") {
                                                ...
                                                if($mfType == "SIP" || $mfType == "SIPR" || $mfType == "SIN" || $mfType == "SINR" || $mfType == "SINRR" )
                                                {
                                                    $mfSubType= "SIP";
                                                }
                                                $transactionType = "Purchase";
                                                ...
                                                //it might be CAMS file, which has trn_type with numbers, so check that $rows
                                                if(strpos($mfType,"P") === 0) {
                                                    $transactionType = "Purchase";
                                                    $mfType = "PIP";
                                                } elseif(strpos($mfType,"DR") === 0) {
                                        ...
                                        if($cell || $cell != '')
                                        {
                                            ...
                                            if(is_object($date)) {
                                                ...
                                                if(strlen($tempDate) == 8) {
                                                    ...
                                                    $insertRow = false;
                                                    $mfMessage = "Date format is not proper (should be dd/mm/yyyy)".strlen($tempDate);
                                                }
                                            }
                                            /*try {
                                                $date = new DateTime(date('Y-m-d', PHPExcel_Shared_Date::ExcelToPHP($cell)));
                        ...
                        if($countRow != 0)
                        {
                            ...
                            if($SIPTRXNNO!='' && $mfSubType=='' &&   (strpos($tempMFType,"P") === 0))   
                            {
                                $mfSubType='SIP';
                            }
                            if($mfSubType=='')
                            ...
                            $dataRows[$countMF] = array(
                                ...
                                'user_id' => $user_id, 'broker_id' => $brokerID,'mutual_fund_sub_type'=>$mfSubType,
                                'from_file'=>$from_file
                            );
                            //var_dump($dataRows[$countMF]);
                            $countMF++;
                           ...
                           /* $remRows[$countRem] = array(
                            ...
                            $folioNum = ""; $clientName = ""; $panNum = ""; $mfType = "";$mfSubType = ""; $purDate = ""; $unit = ""; $schemeName = ""; $prodCode = "";
                            $nav = ""; $adj = ""; $clientId = ""; $familyId = ""; $adjRefNum = ""; $balUnit = ""; $amount = ""; $dpo = 0; $schemeId="";
                            $adj_flag = 0; $tempMFType = ""; 
							$amc = ""; $arn = ""; $sub_arn = ""; $cheque_no = ""; $cheque_bank = ""; $account_no = ""; $ref_no = ""; $rej_ref_no = ""; $trxn_mode = "";
							$SIPTRXNNO="";
    ...
    //Amit
    function auto_mf_valuation_historical($brokerID,$reportDate,$familyID,$clientID,$type)
    {
        if($type=='family')
        {
            $this->mf->valuation_live_units_historical_f($brokerID, $reportDate,$familyID);     
        }
        else
        {
            $this->mf->valuation_live_units_historical_c($brokerID, $reportDate,$clientID);     

        }
        
        $where_condition=array("brokerID"=>$brokerID, "reportDate"=>$reportDate);
        $this->mf->mf_calculate_divp_historical($where_condition);
        $this->mf->mf_calculate_divr_historical($where_condition);
        $this->mf->mf_update_c_nav_historical($where_condition);
        
      
    }
    
    function auto_mf_valuation_delete_op_historical($brokerID,$reportDate,$clientID)
    {
        
        $this->mf->mf_calculate_live_units_delete_op_historical_c($brokerID, $reportDate,$clientID);     
        $where_condition=array("brokerID"=>$brokerID, "reportDate"=>$reportDate);
        $this->mf->mf_calculate_divp_delete_op_historical($where_condition);
        $this->mf->mf_calculate_divr_delete_op_historical($where_condition);
        
        $where_condition=array("brokerID"=>$brokerID, "reportDate"=>$reportDate,"clientID"=>$clientID);
        
        $this->mf->mf_update_c_nav_delete_op_historical($where_condition);
    }
    
    function get_mf_report_historical1()
    {
        $family_id = $this->input->post('famName');
        $client_id = $this->input->post('client_id');
        ...
        $scheme_summary=$this->input->post('scheme_summary');
        $net_investmet=$this->input->post('net_investmet');
        $hide_nav_date=$this->input->post('hide_nav_date');
        $cap_detail=$this->input->post('cap_detail');
        ...
        if($flag==1)
        {
            ...
            $client_wise_summary_by_year = '';
             if($report_type != 'folio_master')
             {
                // calculate current nav and valuation        
                $this->auto_mf_valuation_historical($brokerID,$reportDate,$family_id,$client_id,$type);
               
             }
            
            $net_inv = $this->mf->get_net_investment_historical($type, $where);
            
            $inv_summary = $this->mf->get_investment_summary_historical($type, $where);
            // for chart called function
            $curr_val_summary = $this->mf->get_current_value_summary_historical($type, $where);    
            
            
             if($report_type == 'clientwise_detail')
             {
                 //Clientwise detail report
                 $mf_rep = $this->mf->get_mutual_fund_report_historical($type, $where);
                 
             }
            if($report_type == 'clientwise_summary' || $report_type == 'scheme_group_type_wise' )
            {
              //Clientwise Summary report
              $client_wise_summary=$this->mf->get_clientwise_summary_historical($type,$where1);
            }
            
            if($report_type == 'clientwise_summary_by_year')
            {
              //Clientwise Summary report
              $client_wise_summary_by_year=$this->mf->get_clientwise_summary_historical($type,$where1);
            }
             if($report_type == 'schemewise_detail')
            {
              //Schemewise Detail Report
              $mf_detail_schemewise = $this->mf->get_mutual_fund_family_detail_schemewise_historical($type,$where1);
            
            }
            if($report_type == 'foilowise_summary')
            {
              //Foliowise summary
              $folio_summary=$this->mf->folio_wise_summary_historical($type,$where1);
            }
            if($report_type == 'sip')
            {
              //SIP
              $sip_rep=$this->mf->get_sip_report_historical($type,$where1);
              // print_r($sip_rep);
            }
            if($report_type == 'folio_master')
            {
              $folio_master_data=$this->mf->follio_master($type,$where2);
            }
            ...
            {
                //Common client summary
                $mf_summary_client = $this->mf->get_mutual_fund_family_summary_portfolio_historical($type,$where1);
            }
            if($typewise)
            {
             //Common typewise summary (Equity ,Debt,Banlanced)
              $mf_summary_typewise = $this->mf->get_mutual_fund_family_summary_typewise_historical($type,$where1);
            ...
            {
              //Comman scheme summary
              $mf_comman_scheme_summary = $this->mf->mf_comman_scheme_summary_historical($type,$where1);
            }
            ...
            if($net_investmet)
            {
              //Comman Net Ivestment(Purchase ,Redemption,Dividend Payout,Net Investment)
                ...
                $mf_tran_amount_date = $this->mf->get_mutual_funds_trancation_amount($type,$where1);
                $mf_summary_net_investment = $this->mf->get_mutual_fund_family_summary_schemewise_net_investment_historical($type,$where1);  
	            ...
	            $date_stamps[] = $temp;
	            //->getTimestamp();
	            $values[] =$mf_summary_net_investment[0]->current_value;
            ...
            $mf_comman_cap_detail_1='';
            $mf_comman_cap_detail_2='';
            if($cap_detail)
            {
                
              $mf_comman_cap_detail_1 = $this->mf->mf_comman_cap_detail_1_historical($type,$where1);
            ...
            if($report_type == 'folio_master')
            {
                $comman_cagr_abs_total = $this->mf->mf_comman_scheme_summary_historical($type,$where2);
            }
            else
            {
             $comman_cagr_abs_total = $this->mf->mf_comman_scheme_summary_historical($type,$where1);   
            }
        ...
        else
        {
          ...
          //  $mf_rep = $this->mf->get_mutual_fund_report($type, $where);
            ...
            {
              //Clientwise Summary report
              $client_wise_summary_by_year=$this->mf->get_clientwise_summary($type,$where1);
            }
             if($report_type == 'schemewise_detail')
            {
              //Schemewise Detail Report
            ...
            if($net_investmet)
            {
              //Comman Net Ivestment(Purchase ,Redemption,Dividend Payout,Net Investment)
               ...
               // echo round(($objPHPExcel->XIRR($values,$date_stamps,0.1)*100),3);die();
                $mf_summary_net_investment[0]->xirr_value=round(($objPHPExcel->XIRR($values,$date_stamps,0.1)*100),3);
        ...
        if(!empty($mf_rep) && !isset($mf_rep['code'])  || !empty($client_wise_summary) && !isset($client_wise_summary['code']) ||
              ...
              !empty($folio_master_data) && !isset($folio_master_data['code']) || !empty($client_wise_summary_by_year) && !isset($client_wise_summary_by_year['code']))

        {
            ...
            $mf_rep_array = array(
                                'mf_rep_data' => $mf_rep,
                                'folio_master_data'=>$folio_master_data,
                                'sip_rep'=>$sip_rep,
                                'capital_gain'=>$capital_gain,
                                'net_inv_data' => $net_inv,
                                'report_info'=>$rep_info, 
                                'inv_sum' => $inv_summary, 
                                'cur_val_sum' => $curr_val_summary,
                                'mf_summary_client'=>$mf_summary_client,
                                'mf_summary_typewise'=>$mf_summary_typewise,
                                'mf_detail_schemewise'=>$mf_detail_schemewise,
                                'mf_summary_net_investment'=>$mf_summary_net_investment,
                                'client_wise_summary'=>$client_wise_summary,
                                'mf_comman_scheme_summary'=>$mf_comman_scheme_summary,
                                'folio_summary'=>$folio_summary,
                                'comman_cagr_abs_total'=>$comman_cagr_abs_total,
                                'client_wise_summary_by_year'=>$client_wise_summary_by_year,
                                'hide_nav_date'=>$hide_nav_date,
                                'mf_comman_cap_detail_1'=>$mf_comman_cap_detail_1,
                                'mf_comman_cap_detail_2'=>$mf_comman_cap_detail_2,
    ...
    function get_mf_report_historical()
    {
        $family_id = $this->input->post('famName');
        $client_id = $this->input->post('client_id');
        $report_type = $this->input->post('type');//Schemwisedetail, Clientwise detail,clientwise summamry,Foliowise  detail,SIP,Capitalgain
        ...
        if($flag==1)
        {
             if($client_id != null && $client_id != '' && ($family_id=='' || $family_id==null))
            {
                $where = array(
                    'clientID'=> $client_id,
                    'brokerID'=> $brokerID,
            ...
            $net_inv = $this->mf->get_net_investment_historical_new($type, $where);
            
            $inv_summary = $this->mf->get_investment_summary_historical_new($type, $where);
            
            // for chart called function
            $curr_val_summary = $this->mf->get_current_value_summary_historical_new($type, $where);    
            
            
             if($report_type == 'clientwise_detail')
             {
                 //Clientwise detail report
                 $mf_rep = $this->mf->get_mutual_fund_report_historical_new($type, $where);
                 
             }
            if($report_type == 'clientwise_summary' || $report_type == 'scheme_group_type_wise' )
            {
              //Clientwise Summary report
              $client_wise_summary=$this->mf->get_clientwise_summary_historical_new($type,$where1);
            }
            
            if($report_type == 'clientwise_summary_by_year')
            {
              //Clientwise Summary report
              $client_wise_summary_by_year=$this->mf->get_clientwise_summary_year_wise_historical_new($type,$where1);
            }
             if($report_type == 'schemewise_detail')
            {
              //Schemewise Detail Report
              $mf_detail_schemewise = $this->mf->get_mutual_fund_family_detail_schemewise_historical_new($type,$where1);
           // print_r($where1);die();
            }
            if($report_type == 'foilowise_summary')
            {
              //Foliowise summary
              $folio_summary=$this->mf->folio_wise_summary_historical_new($type,$where1);
            }
            if($report_type == 'sip')
            {
              //SIP
              $sip_rep=$this->mf->get_sip_report_historical_new($type,$where1);
              // print_r($sip_rep);
            }
            if($report_type == 'folio_master')
            {
              $folio_master_data=$this->mf->follio_master_new($type,$where2);
            }
            if($report_type == 'capital_gain')
            {
              if($client_id != null && $client_id != '')
              ...
              $capital_gain=$this->mf->capital_gain_new($type,$where_con);
              // print_r($capital_gain);
            ...
            {
                //Common client summary
                $mf_summary_client = $this->mf->get_mutual_fund_family_summary_portfolio_historical_new($type,$where1);
            }
            if($typewise)
            {
             //Common typewise summary (Equity ,Debt,Banlanced)
              $mf_summary_typewise = $this->mf->get_mutual_fund_family_summary_typewise_historical_new($type,$where1);
            ...
            {
              //Comman scheme summary
              $mf_comman_scheme_summary = $this->mf->mf_comman_scheme_summary_historical_new($type,$where1);
            }
            ...
            if($net_investmet)
            {
              //Comman Net Ivestment(Purchase ,Redemption,Dividend Payout,Net Investment)
                ...
                $mf_tran_amount_date = $this->mf->get_mutual_funds_trancation_amount_new($type,$where1);
                $mf_summary_net_investment = $this->mf->get_mutual_fund_family_summary_schemewise_net_investment_historical_new($type,$where1);  
                foreach($mf_tran_amount_date as $s2) 
                {
	                $values[] = $s2->amount;
	                //$dates[] = DateTime::createFromFormat("Y-m-d",$s2->purchase_date)->format("m-d-Y");
            ...
            if($cap_detail)
            {
              ...
              $mf_comman_cap_detail_1 = $this->mf->mf_comman_cap_detail_1_historical_new($type,$where1);
              
              $mf_comman_cap_detail_2 = $mf_comman_cap_detail_1;
              $mf_summary_detail_for_chart = $this->mf->mf_summary_detail_historical($type,$where1);
              
            }
            //print_r($folio_master_data);
            if($report_type == 'folio_master')
            {
                $comman_cagr_abs_total = $this->mf->mf_comman_scheme_summary_historical_new($type,$where2);
            }
            else
            {
             $comman_cagr_abs_total = $this->mf->mf_comman_scheme_summary_historical_new($type,$where1);   
            }
        ...
        else
        {
            ...
            $net_inv = $this->mf->get_net_investment_new($type, $where);
            $inv_summary = $this->mf->get_investment_summary_new($type, $where);
            $curr_val_summary = $this->mf->get_current_value_summary_new($type, $where);
            if($report_type == 'clientwise_detail')
            {
                //Clientwise detail report
                $mf_rep = $this->mf->get_mutual_fund_report_new($type, $where);
            }
            
            if($report_type == 'clientwise_summary' || $report_type == 'scheme_group_type_wise' )
            {
              //Clientwise Summary report
              $client_wise_summary=$this->mf->get_clientwise_summary_new($type,$where1);
            }
            
            if($report_type == 'clientwise_summary_by_year')
            {
              //Clientwise Summary report
              $client_wise_summary_by_year=$this->mf->get_clientwise_summary_year_wise($type,$where1);
            }
             if($report_type == 'schemewise_detail')
            {
              //Schemewise Detail Report
            //  print_r($where1);
              $mf_detail_schemewise = $this->mf->get_mutual_fund_family_detail_schemewise_new($type,$where1);
            // print_r($mf_detail_schemewise);die();
            }

            if($report_type == 'foilowise_summary')
            {
              //Foliowise summary
              $folio_summary=$this->mf->folio_wise_summary_new($type,$where1);

            }
            if($report_type == 'sip')
            {
              //SIP
              $sip_rep=$this->mf->get_sip_report_new($type,$where1);
              // print_r($sip_rep);
            }
         
            if($report_type == 'folio_master')
            {
              $folio_master_data=$this->mf->follio_master_new($type,$where1);
            }
            if($report_type == 'capital_gain')
            {
              if($client_id != null && $client_id != '')
            ...
            {
                  //Common client summary
                $mf_summary_client = $this->mf->get_mutual_fund_family_summary_portfolio_new($type,$where1);
            }
            if($typewise)
            {
            //Common typewise summary (Equity ,Debt,Banlanced)
              $mf_summary_typewise = $this->mf->get_mutual_fund_family_summary_typewise_new($type,$where1);
            ...
            {
              //Comman scheme summary
              $mf_comman_scheme_summary = $this->mf->mf_comman_scheme_summary_new($type,$where1);
            }

            if($net_investmet)
            {
              //Comman Net Ivestment(Purchase ,Redemption,Dividend Payout,Net Investment)
                $mf_tran_amount_date = $this->mf->get_mutual_funds_trancation_amount_new($type,$where11);
                $mf_summary_net_investment = $this->mf->get_mutual_fund_family_summary_schemewise_net_investment_new($type,$where1);
                foreach($mf_tran_amount_date as $s2) 
                {
	                $values[] = $s2->amount;
	                //$dates[] = DateTime::createFromFormat("Y-m-d",$s2->purchase_date)->format("m-d-Y");
            ...
            $comman_cagr_abs_total = $this->mf->mf_comman_scheme_summary_new($type,$where1);
...

        {
            ...
            $mf_rep_array = array(
                                ...
                                'mf_comman_cap_detail_1'=>$mf_comman_cap_detail_1,
                                'mf_comman_cap_detail_2'=>$mf_comman_cap_detail_2,
                                'mf_summary_detail_for_chart'=>$mf_summary_detail_for_chart,
    ...
    function get_mf_broker_report()
    {
        ...
        else if($type=='aum_report') {
           $mf_aum_report = $this->mf->get_aum_report(array('broker_id' =>$brokerID));
           $mf_aum_report_for_chart = $this->mf->get_aum_report_for_chart(array('broker_id' =>$brokerID));
           
        }
        else if($type=='all_summary_report') {
             $all_wise_summary=$this->mf->get_all_summary(array('broker_id' =>$brokerID));
        }
        else if($type=='family_aum_report') {
            $family_wise_summary = $this->mf->get_family_wise_summary_aum(array('broker_id' =>$brokerID));
        }
        else if($type=='detail_aum_report')
        {
            $detail_aum_report=   $this->mf->detail_aum_report_from_file(array('broker_id' =>$brokerID));
        }
        $status =false;
        $logo='';
        ...
        if(!empty($mf_aum_report) ||!empty($all_wise_summary) || !empty($family_wise_summary)|| !empty($detail_aum_report)  ||  (!empty($month) || !empty($quarterly) || !empty($half_yr) || !empty($yearly)) )
        {
            ...
            $mf_rep_array = array('mf_aum_report' => $mf_aum_report,
                                'mf_aum_report_for_chart'=>$mf_aum_report_for_chart,
                                  'rep_info'=>$rep_info,
                                  'mf_sip_report'=>$mf_sip_report,
                                  'all_wise_summary'=>$all_wise_summary,
                                  'family_wise_summary'=>$family_wise_summary,
                                  'detail_aum_report'=>$detail_aum_report
                                  );
	...
	//error_reporting(0);
        ...
        if(!(isset($_POST['htmlData']) && !empty($_POST['htmlData']))) {
            ...
            // set default header data
            //$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE.' -Admin', PDF_HEADER_STRING);
            $pdf->SetHeaderData($logo, 30, $title, '');
            ...
            if(isset($_POST['chart_1']))
            {
                ...
                $pdf->setImageScale(PDF_IMAGE_SCALE_RATIO);
                $imgdata = base64_decode($_POST['chart_1']);    
             
            $pdf->SetXY($x, $y);
            $pdf->Image('@'.$imgdata, '', '', 135, 80, '', '', 'T', false, 300, '', false, false, 2, false, false, false);
            
            }
            if(isset($_POST['chart_1']))
            {
            
                if(isset($_POST['chart_2']))
                {
                    $x+=140;    
                }
                $imgdata = base64_decode($_POST['chart_2']);    
                $pdf->SetXY($x, $y+5);
                $pdf->Image('@'.$imgdata, '', '', 135, 80, '', '', 'T', false, 300, '', false, false, 2, false, false, false);
                $y+=85;
            }
     ...
     function export_to_pdf_with_three_chart()
    {
        ini_set('max_execution_time', 0);
	...
	//error_reporting(0);
        ...
        if(!(isset($_POST['htmlData']) && !empty($_POST['htmlData']))) {
            ...
            if(isset($_POST['chart_1']))
            {
                ...
                $pdf->SetXY($x, $y);
                $pdf->Image('@'.$imgdata, '', '', 80, 50, '', '', 'T', false, 300, '', false, false, 2, false, false, false);
            ...
            if(isset($_POST['chart_1']))
            {
                ...
                $imgdata = base64_decode($_POST['chart_3']);    
                $pdf->SetXY($x, $y);
                $pdf->Image('@'.$imgdata, '', '', 80, 50, '', '', 'T', false, 300, '', false, false, 2, false, false, false);
                $y+=70;
            }
