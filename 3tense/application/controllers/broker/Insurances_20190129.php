...
class Insurances extends CI_Controller{
    ...
    public function ajax_list()
    {
        ...
        'prem_paid_till_date', 'fund_option1', 'fund_option2');
    ...
    function edit_ins()
    {
        ...
        $data['hPaidupDate'], $data['hMatDate']);
                ...
                else
                {
                    ...
                    foreach($excelData as $rows)
                    {
                        ...
                        if($countRow != 0)
                        {
                            ...
                            /* Commented out, uncomment if needed - Salmaan - 02-05-16
                            //set all dates to compare
                            $matDateTemp = strtotime($matDate); $fupDateTemp = strtotime($fupDate);
                            $paidUpDateTemp = strtotime($paidUpDate); $todayTemp = strtotime('today');

                            if($matDateTemp < $todayTemp) {
                                $key = array_search('Matured', array_column($statuses, 'status'));
                            } elseif($fupDateTemp < $paidUpDateTemp) {
                                $key = array_search('Lapsed', array_column($statuses, 'status'));
                            } elseif($fupDateTemp > $paidUpDateTemp) {
                                $key = array_search('Paid up', array_column($statuses, 'status'));
                            } else {
                                $key = false;
                            } */
                            ...
                            /* Commented out, uncomment if needed - Salmaan - 02-05-16
                            if(($todayTemp < $graceDueDateTemp) && ($todayTemp > $fupDateTemp)) {
                                $key = array_search('Grace', array_column($statuses, 'status'));
                            }
                            //if key is not false, then set status_id
                            if($key) {
                                $status = intval($statuses[$key]['status_id']);
                            } else {
                                $status = 1;
                            }*/
                            ...
                            /* Commented out & added new by Salmaan - 02/05/16
    ...
    function scheme_import()
    {
        ...
        $arrCellLabels = array('Scheme Name'=>'R','Scheme Type'=>'R','Product Code'=>'R','Other'=>'N','Other2'=>'N','ISIN'=>'R');
                        ...
                        if(!$skipProcessing)
                        {
                            ...
                            else
                            {
                                ...
                                foreach($arrScheme as $key => $val)
                                {
                                    ...
                                    if($arrCellLabels[$key] == 'R' && (is_null($val) || empty($val) || $val == 'NULL'))
                                    {
                                ...
                                if($blnProcessRow)
                                {
                                    ...
                                    {
                                        //check if a scheme already exists in system
                                         $condition = array(
                                        ...
                                        else
                                        {
                                            ...
                                            $data = array('scheme_name'=>$scheme_name,'scheme_type_id'=>$type,'prod_code'=>$product_code,'isin'=>$isin_num,'other'=>$other,'other2'=>$other2);
