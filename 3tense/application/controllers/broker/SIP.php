...
class SIP extends CI_Controller{
    ...
    function MonthlySIPBook($err_data=null)
    {
        ...
        $header['title'] = 'Import Monthly SIP Book Details';
        $header['css'] = array(
          'assets/users/plugins/datatables/css/jquery.dataTables.min.css',
          'assets/users/plugins/form-select2/select2.css',
          'assets/users/plugins/pines-notify/jquery.pnotify.default.css'
        ...
        $this->load->view('broker/common/header', $header);
        $data['import_data'] = $err_data;
        $this->load->view('broker/MonthlySIPBook', $data);
        $this->load->view('broker/common/notif');
        $this->load->view('broker/common/footer');
    }
    function Import_MonthlySIPBook()
    {
        ...
        if (isset($_POST['Import']))
        {
            ...
            if(isset($_FILES["import_monthly_Sip"]) AND ($_FILES["import_monthly_Sip"]["type"]=="application/excel"||$_FILES["import_monthly_Sip"]["type"]=="application/vnd.ms-excel"||$_FILES["import_monthly_Sip"]["type"]=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
            {
                if (isset($_FILES["import_monthly_Sip"]))
                {
        
                  if ($_FILES["import_monthly_Sip"]["name"] == '')
                  {
                  ...
                  else 
                  {
                    ...
                    $file = $_FILES["import_monthly_Sip"]["tmp_name"];
                    ...
                    foreach($excelData as $rows)
                    {
                      ...
                      foreach($rows as $cell)
                      {
                        if($countRow == 0)
                        {
                          ...
                          if(strtoupper($cell)=='DATE'||strtoupper($cell)=='AMOUNT')
                          {
                            $message='match';
                        ...
                        else
                        {
                          ...
                          if($insertRow)
                          {
                                if(strtoupper($dataColumns[$countCell]) === 'AMOUNT' )
                                {
                                    if($cell || $cell != '')
                                    {
                                        $Account= $cell;
                                ...
                                else if(strtoupper($dataColumns[$countCell]) === 'DATE')
                                {
                                    ...
                                    if($cell || $cell != '')
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                            $insertRow = false;
                                            $impMessage = "Date format is not proper (should be dd/mm/yyyy)";
                                            }
                                        }
                                    }
                                    else
                    ...
                    if($countRow != 0)
                    {
                        if(!$insertRow)
                        {
                            ...
                            $dataRows['add_SIP_list'] = array (
                             'amount'=>$Account,
                             'sip_date'=> date($SIP_date),
                             'broker_Id'=> $brokerID,
                             'CreatedBy'=> $user_id
                            );
                       
                            $assets_record=array('broker_id'=>$brokerID,'sip_date'=>date($SIP_date));

                            if($isDuplicateSIP=$this->sp->check_duplicate_sip_monthly_book($assets_record))
                            {
        
                                $SIP_Book_id=$isDuplicateSIP->Id;
                                $inserted =$this->sp->update_duplicate_sip_montly_book($dataRows['add_SIP_list'],$SIP_Book_id);
                                $uploadedStatus = 1;
                            }
                            else
                            {
                                
                                $inserted = $this->sp->add_sip_monthly_book($dataRows['add_SIP_list']);
                                $assets_id=$inserted;
                    ...
                    if($dataRows)
                    {
                        if(is_array($inserted))
                        {
                            ...
                            $this->common->last_import('SIP monthly book Details', $brokerID, $_FILES["import_monthly_Sip"]["name"], $user_id);
                            if($uploadedStatus != 2) {
                                $uploadedStatus = 1;
                                $message = "SIP Monthly Book Details Uploaded Successfully";
                            }
                        }
                    }
                  ...
                  foreach($rows as $cell)
                  {
                    ...
                    else
                    {
                      ...
                      if($insertRow)
                      {
                                ...
                                $assets_record=array('scheme_id'=>$scheme_id,'start_date'=>$start_date,'folio_no'=>$folio_id,'client_id'=>$client_id,'ref_number'=> $auto_trno);
                                ...
                                if($isDuplicateSIP=$this->sp->check_duplicate_sip($assets_record))
                                {
                                 ...
                                 $inserted =$this->sp->update_duplicate_sip($dataRows['add_SIP_list'],$assets_record);
                    ...
                    //print_r($scheme_id->scheme_id);
                                ...
                                //$assets_record=array('scheme_id'=>$scheme_id,'start_date'=>$start_date,'folio_no'=>$folio_id,'client_id'=>$client_id);
                                 $assets_record=array('scheme_id'=>$scheme_id,'start_date'=>$start_date,'folio_no'=>$folio_id,'client_id'=>$client_id,'ref_number'=> $ref_number);
                                ...
                                foreach($excelData as $rows)
                                {
                                  ...
                                  foreach($rows as $cell)
                                  {
                                   ...
                                   if($countRow == 0)
                                   {
                                      ...
                                      /* extra columns - Salmaan - 2017-07-19 */
                                      || strtoupper($cell)=='BANK_NAME' || strtoupper($cell)=='BANK_ACCNO' || strtoupper($cell)=='IFSC_CODE' 
                                      || strtoupper($cell)=='DEBIT_TYPE' || strtoupper($cell)=='UMRN_NO' || strtoupper($cell)=='ACH_STATUS'
                              ...
                              foreach($excelData as $rows)
                              {
                                ...
                                foreach($rows as $cell)
                                {
                                  ...
                                  if($countRow == 0)
                                  {
                                   $cell = str_replace(array('.'), '', $cell);
                                   $cell = str_replace(array("'"),'', $cell);
                                   if(strtoupper($cell)=='SR NO' ||strtoupper($cell)=='TRXN MODE'||strtoupper($cell)=='GROUP'|| strtoupper($cell)=='TRXNNUMBER' ||
                                    strtoupper($cell)=='PARTNER/EMPLOYEE' || strtoupper($cell)=='PARTNER/EMPLOYEE' || 
                                    ...
                                    strtoupper($cell)=='START DATE'||strtoupper($cell)=='SECOND DATE' ||strtoupper($cell)=='END DATE' ||strtoupper($cell)=='NAV' ||strtoupper($cell)=='PURUNITS'
                                    ||strtoupper($cell)=='AMOUNT'|| strtoupper($cell)=='FREQUENCY' || strtoupper($cell)=='INSTALLMENT(S)' || strtoupper($cell)=='POST DATE'  
                              ...
                              $dataRows['add_SIP_list'] = array (
                               ...
                               'added_on'=> date($added_on)
                                  ...
                                  //$assets_record=array('ref_number'=>$ref_number,'folio_no'=>$folio_id,'client_id'=>$client_id);
                                   ...
                                   $assets_record=array('scheme_id'=>$scheme_id,'start_date'=>$start_date,'folio_no'=>$folio_id,'client_id'=>$client_id,'ref_number'=> $ref_number);
                                          //$isDuplicateSIPtest=$this->sp->check_duplicate_sip_test($assets_record);
                                          //var_dump($isDuplicateSIPtest);
                                   if($isDuplicateSIP=$this->sp->check_duplicate_sip($assets_record))
                                   {
                                      ...
                                      foreach($rows as $cell)
                                      {
                                       ...
                                       if($countRow == 0)
                                       {
                                        ...
                                        if(strtoupper($cell)=='PRODUCT'||strtoupper($cell)=='SCHEME'||strtoupper($cell)=='FOLIO_NO'||
                                          ...
                                          strtoupper($cell)=='AMC_CODE' || strtoupper($cell)=='USER_CODE' || 
                                          strtoupper($cell)=='PACKAGE_NA' || strtoupper($cell)=='SPECIAL_PR' || strtoupper($cell)=='SUBTRXNDES' || strtoupper($cell)=='PAUSE_FROM' || strtoupper($cell)=='PAUSE_TO_D')
                                        {
                                          $message='match';
                                          $dataColumns[$countCell] = $cell;
                                          $countCell++;
                                        ...
                                        {
                                          $message = 'Columns Specified in file is not in correct format';
                                          $uploadedStatus = 0;
                                          break;
                                                    //die();
                                        }
            ...
            foreach($rows as $cell)
            {
              ...
              //print_r($scheme_id->scheme_id);
                                        ...
                                        if($isDuplicateSIP=$this->sp->check_duplicate_sip($assets_record))
                                        {
                                         ...
                                         $client_id=$isDuplicateSIP->client_id;
                                         $assets_id=$isDuplicateSIP->asset_id;
                                                                               //echo 'update-'; var_dump($assets_id);
                                       ...
                                       else
                                       {
                                         ...
                                         $inserted = $this->sp->add_sip($dataRows['add_SIP_list']);
                                         $assets_id=$inserted;
                                                                         //echo 'insert-'; var_dump($assets_id);
                                         $AddInterest = array('start_date'=> date($start_date),'end_date'=> date($end_date),'frequency'=>$frequency,'installment_amt'=> $installment_amt,'broker_id'=>$brokerID,'asset_id'=>$assets_id);
                                         $status=$this->add_sipinterest_details($AddInterest, $assets_id);
          ...
          if($insertRow)
          {
                                                       ...
                                                       else
                                                       {
                                                        ...
                                                        $impMessage = "Scheme cannot be empty";
                                                              ...
                                                              else if(strtoupper($dataColumns[$countCell]) === 'ACCOUNT_N1' || strtoupper($dataColumns[$countCell]) === 'ACCOUNT NUMBER')//folio_id
                                                              {
                                                              ...
                                                              else
                                                              {
                                                                ...
                                                                else if(strtoupper($dataColumns[$countCell]) === 'SIP_REG_14' || strtoupper($dataColumns[$countCell]) === 'SIP REGISTERED DATE' || strtoupper($dataColumns[$countCell]) === 'REGDATE')//added on date
                                                                {
                                                                  ...
                                                                  else if(strtoupper($dataColumns[$countCell]) === 'SIP TRANSACTION NUMBER' || strtoupper($dataColumns[$countCell]) === 'SIP_TXN_NO')//ref_no
                                                                  {
                                                                    if($cell || $cell != '')
                                                                    {
                                                                      $ref_number = $cell;
