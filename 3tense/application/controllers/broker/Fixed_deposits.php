...
class Fixed_deposits extends CI_Controller {
    ...
    private function add_interest_details($data, $transID)
    {
        ...
        try
        {
            if($int_round_off == 1)
            {
                ...
                } elseif(strtoupper($int_mode) == "HALF-YEARLY") {
                    if($month <= 6)
                        $int_date = $year.'-06-30';
                    else
                        $int_date = ($year).'-12-31';
                ...
                } elseif(strtoupper($int_mode) == "QUARTERLY") {
                    if($month <= 3)
                    {
                        $int_date = $year.'-03-31';
                    }
                ...
                } elseif(strtoupper($int_mode) == "MONTHLY") {
                    $int_date_temp = new DateTime($year.'-'.$month.'-1');
    ...
    function get_fd_report()
    {
        ...
        if($client_id != null && $client_id != '')
        {
            ...
            $list = $this->fd->get_fixed_deposit_mat("fdt.broker_id ='".$brokerID."' and fdt.maturity_date>=curdate() and  fdt.client_id='". $client_id[0]."' ");
        }
        else
        {
            $type = 'family';
           ...
           $list = $this->fd->get_fixed_deposit_mat("fdt.broker_id ='".$brokerID."' and fdt.maturity_date>=curdate() and c.family_id='". $family_id[0]."' ");
        }
        $logo = "";
        $status = false;
        ...
        if(!empty($fd_rep))
        {
            ...
            $fd_rep_array = array('fd_rep_data' => $fd_rep, 
    ...
    function export_to_pdf()
    {
     ...
     function export_to_pdf_with_chart_and_table()
    {
        ini_set('memory_limit', '512M');
        //echo html_entity_decode($_POST['htmlData']);
        if(!(isset($_POST['htmlData']) && !empty($_POST['htmlData']))) {
            ...
            $x=10;$y=50;
            if(isset($_POST['summary_data']))
            {
                $pdf->writeHTML($css_data.$summarydata, true, false, true, false, '');
            ...
            $pdf->SetXY(10, $y);
            $pdf->writeHTML($css_data.$fd_data, true, false, true, false, '');
            // reset pointer to the last page
            $pdf->lastPage();
            ob_end_clean();
                              ...
                              foreach($excelData as $rows)
                              {
                                  ...
                                  if($countRow != 0)
                                  {
                                     ...
                                     $interval = $mat_date->diff($issue_date);
                                      ...
                                      $total_days = $interval->days; //added by Salmaan - 2017-07-28
				      ...
				      if(is_array($inserted) && isset($inserted['message']))
                                      {
                                          ...
                                          $imp_data[$countErrRow][1] = $PanNum;
                                          $imp_data[$countErrRow][2] = $referenceNumber;
                                          $imp_data[$countErrRow][3] = 'Error in adding. '.$inserted['message'];
