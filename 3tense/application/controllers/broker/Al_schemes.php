...
class Al_schemes extends CI_Controller{
    function __construct()
    {
        ...
        $this->load->model('Al_schemes_model', 'pro');
    ...
    function index()
    {
        ...
        $header['title'] = 'Asset or Liability Scheme Master';
        $header['js'] = array(
        ...
        $this->load->view('broker/asset_liability/al_scheme');
    ...
    function ajax_al_list()
    {
        ...
        $list = $this->pro->get_schemes($where);
        $data = array();
        $num = 10;
        ...
        foreach($list as $scheme)
        {
            ...
            if($scheme->broker_id != null) {
                ...
                onclick="edit_al_scheme('."'".$scheme->scheme_id."'".')"><i class="fa fa-pencil"></i></a>
                <a class="btn btn-sm btn-danger" href="javascript: void(0)" title="Delete"
                onclick="delete_al_scheme('."'".$scheme->scheme_id."'".')"><i class="fa fa-trash-o"></i></a>';
                }
    ...
    function add_scheme()
    {
        ...
        $condition = 'scheme_name = "'.trim($this->input->post('scheme_name')).'"
            AND (broker_id = "'.$this->session->userdata('broker_id').'" OR broker_id IS NULL)';
        $isDuplicate = $this->com->check_duplicate('scheme_name', 'al_schemes', $condition);
        if($isDuplicate == null)
        {
            try
            {
                $status = $this->pro->add_scheme($data);
                ...
                $message = array(
                    ...
                    'status' => 'new',
                    'text' => 'Asset and Liability new scheme "'.$this->input->post('scheme_name').'" added successfully'
                );
            }
        ...
        else
        {
            $message = array(
                'type' => 'error',
                'title'=>'Error on Adding Scheme!',
                'status' => 'duplicate',
                'text' => 'Asset and Liability scheme "'.$this->input->post('scheme_name').'" already exists'
            );
        }
    ...
    {
        $condition = array('scheme_id' => $this->input->post('id'), 'broker_id' => $this->session->userdata('broker_id'));
        $data = $this->pro->get_scheme_details($condition);
    ...
    function update_scheme()
    {
        ...
        $condition = 'scheme_name = "'.trim($this->input->post('scheme_name')).'"
            AND scheme_id != "'.$this->input->post('scheme_id').'"
            AND (broker_id = "'.$this->session->userdata('broker_id').'" OR broker_id IS NULL)';
        $isDuplicate = $this->com->check_duplicate('scheme_name', 'al_schemes', $condition);
        if(!$isDuplicate)
        {
            try
            {
                $status = $this->pro->update_scheme(array('scheme_id' => $this->input->post('scheme_id'), 'broker_id' => $brokerID), $data);
                //if there is any error
                if(isset($status['code']))
                {
                    throw new Custom_exception();
                ...
                $message = array(
                    ...
                    'status' => 'new',
                    'text' => 'Asset and Liability scheme "'.$this->input->post('scheme_name').'" updated successfully'
                );
            }
            catch(Custom_exception $e)
            {
        ...
        else
        {
            $message = array(
                'type' => 'error',
                'title'=> 'Error on updating Scheme!',
                'status' => 'duplicate',
                'text' => 'Asset and Liability scheme "'.$this->input->post('scheme_name').'" already exists'
            );
        }
    ...
    function delete_scheme()
    {
        ...
        try
        {
            $status = $this->pro->delete_scheme(array('scheme_id' => $this->input->post("id"), 'broker_id' => $brokerID));
            //if there is any error
            if(isset($status['code']))
            {
                throw new Custom_exception();
    ...
    function get_scheme_dropdown()
    {
        $brokerID = $this->session->userdata('broker_id');
        $result = $this->pro->get_scheme_broker_dropdown('broker_id = "'.$brokerID.'" OR broker_id IS NULL');
