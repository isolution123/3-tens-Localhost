...
class Users extends CI_Controller {
    ...
    public function logout()
    {
      ...
      unset($_SESSION['popup']); //Akshay Karde - 2017-04-22 - for dashboard popup modal - system changes/maintenance
      //unset($_SESSION);
    ...
    public function user_authication()
    {

      
            
            if ($this->input->post('btn_authentication') == "Login")
            {

                $auth_code = $this->input->post('auth_code');
                $code=$this->session->userdata['auth_code'];
                ...
                if($auth_code==$code)
                {
                    ...
                    if($result)
                    {
                      ...
                      $this->session->set_userdata($sess_user1);
        ...
        else
        {
            ...
            if ($this->input->post('btn_login') == "Login")
            {
                ...
                if($result)
                {
                    ...
                    if($broker_id=='0004'  && strtolower($username)!='amit' && $username!='dhaval'&&  strtolower($username)!='isolutions1')
                    {
                        ...
                        //if(($query = $this->Send_model->authentication_mail('isolutions942@gmail.com',$result->name,$auth_code,$broker_id)) === true)
                        {
    ...
    function manage_users()
     {
       if(empty($this->session->userdata['user_id']))
       {
        ...
        $limit = $this->Users_model->get_limit($brokerID);
        $count = $this->Users_model->count_user($brokerID);
        $limit=intval($limit->user_limit);
        ...
        if($count >= $limit)
        {
          ...
          //redirect('broker/Users/manage_users');

          if($this->session->userdata('user_id') == $this->session->userdata('broker_id'))
          {
              //data to pass to header view like page title, css, js
              $header['title']='Manage Users';
              $dt['limit']=array('Limit excedd');
              $header['css'] = array(
                  'assets/users/plugins/datatables/css/jquery.dataTables.min.css',
              ...
              //load views
              $this->load->view('broker/common/header', $header);
              $this->load->view('broker/user/manage_users',$dt);
              $this->load->view('broker/common/notif');
              $this->load->view('broker/common/footer');
          } else {
    ...
    public function add_user()
     {
              ...
              $condition = "`username` = '".trim($user_data['username'])."'";
                  ...
                  try
                  {
                      ...
                      if(!empty($admin)) {
                          ...
                          if($inserted && !is_array($inserted)) {
                              ...
                              $body .= "<p>Password: <span style='font-weight: bold;'>".$pass."</span></p>";
    ...
    public function update_user()
    {
        ...
        //$user_data['broker_id'] = $broker_id;
        $condition = "`username` = '".trim($user_data['username'])."' AND `id` <> '".$user_data['id']."'";
    ...
    function eod() {
        ...
        $conditions = '(reminder_type <> "Shares Negative Balance" AND reminder_status = "Complete" AND reminder_date IS NOT NULL
                            AND DATEDIFF(reminder_date,CURDATE()) = 0 AND (client_id IS NOT NULL OR client_name IS NOT NULL) AND broker_id = "'.$broker_id.'")
