...
$broker_pwd=array();
foreach($cams_auto_import as $key => $value){
//  echo $cams_auto_import[$key];
/*for CAMS client Folio file auto import*/
         if($cams_auto_import[$key]=="WBR9. Investor Static details feed - CRMS Format, Request Id"){   //cams if of folio master files;
      $emails = imap_search($inbox, 'SUBJECT "WBR9. Investor Static details feed - CRMS Format, Request Id" ');
          ...
          //  echo " Mails for today";
            ...
            // echo"<pre>";print_r($today_mail);exit;
             if(isset($today_mail) && !empty($today_mail)){
               foreach($today_mail as $tm){
...
//echo"<pre>";print_r($temp);exit;
  foreach($temp as $t){
    $req_id=trim($this->GetBetween('Report Request No:','This request=',$t));
  //echo $req_id;//exit;
    $final_link=str_replace(array("\n","\r\n","\r"), '', str_replace('=','',$this->GetBetween('Link :','The zip file',$t)));
...
// echo"<pre>";print_r($to_part);
// exit;
    $lcnt=0;
  foreach($temp1 as $fkey=>$fval){
    echo $lcnt;
    $link=$temp1[$fkey];
  ...
  // function get_attachment_from_mail()
  // {
        ...
        // exit;
         ...
         // $unzip=true;//comment this line on server and uncomment below line on server
              $location="downloads/Folio_master_list/cam_folder/";
               $brokerName=$to_part[$fkey];
               
               //below code added by Salmaan - 2017-11-02
               $brokerID1 = $this->usr->get_users(array("mailback_mail"=>$brokerName));
               var_dump($brokerID1);
               if(!empty($brokerID1)) {
                   $this->session->set_userdata(array(
                       'brokerID'=>$brokerID1[0]->id,
                       'userID'=>$brokerID1[0]->id,
                       ));
               } else {
                   //do nothing for now
               }
               
               $pwd=$this->Clients_model->get_cams_RTA_import_password($brokerName);
               //below code added by Salmaan - 2017-11-01
               if(!$pwd) {
                   echo 'No user found for mailback ID <br/>';
                   continue;
               }
        ...
        $unzip = $this->extract_archive($filepath,$location,$pwd);
          if(!$unzip) {
            ...
            $today=$date->format('d-m-Y');//date('d-m-Y');
              ...
              $SearchDate = $searchTemp->format('d-m-Y');
            $SearchDate=str_replace('-','',$SearchDate);
    //  print_r($today_cam);exit;
      ...
      // echo "Matched\n";
       $read_arr[]=$file;
              ...
              print_r($read_arr);//exit;
                ...
                if(strpos($read_arr[$i], 'downloads/Folio_master_list/cam_folder/')=== false){
                                                $read_arr[$i]='downloads/Folio_master_list/cam_folder/'.$read_arr[$i];
                                              }else{
                                                $read_arr[$i]=$read_arr[$i];
                                              }
            //echo 'Clients import';
            $allset[$key][$i]= $this->clients_import($successFlag,$read_arr[$i],$passfileType);
            }
...
// for mutual fund transaction  import
   if($cams_auto_import[$key]=="WBR2. Investor Transactions for a Period, Request Id"){
     $temails = imap_search($inbox, 'SUBJECT "WBR2. Investor Transactions for a Period, Request Id" ');
         ...
         foreach($t_temp as $tt){
           $req_id=trim($this->GetBetween('Report Request No:','This request was',$tt));
           $tfinal_link=str_replace(array("\n","\r\n","\r"), '', str_replace('=','',$this->GetBetween('https://mailback.camsonline.com/mailback_result/','The zip file',$tt)));
            ...
            if(strpos($tfinal_link1,'/mailback_result/')){
                  //  echo"in hell";
                     $t_temp1[]=$tfinal_link1;   //array of final link having zip file for mf transaction of  todays date
                      ...
                      $tproblem_message[]="file link is not available in the mail with Request ID:".$req_id."And date:".$tcomp_today_date;
                  }

         }
       //echo"<pre>";print_r($t_temp1);exit;
           $tlcnt=0;
           foreach($t_temp1 as $tskey=>$tsval){
             $link=$t_temp1[$tskey];
             ...
             //set the url from where to download
               ...
               if(!empty($brokerID1)) {
                   ...
                   //do nothing for now
                      ...
                      $pwd1=$this->Clients_model->get_cams_RTA_import_password($brokerName);
             ...
             if(preg_match('#^' . $tSearchDate . '.*?\.dbf$#', $file)) {
               ...
               // echo "Matched\n";
                $tread_arr[]=$file;
                         ...
                         $successFlag=true;
                         $passfileType='cams_excel';
                         if(strpos($tread_arr[$i], 'downloads/MF_Transaction_list/cam_folder/')=== false){
                                                         $tread_arr[$i]='downloads/MF_Transaction_list/cam_folder/'.$tread_arr[$i];
                                                       }else{
                                                         $tread_arr[$i]=$tread_arr[$i];
                                                       }
                          echo "auto file=".$tread_arr[$i];
                      //echo 'MF import';
                      $allset[$key][$i]= $this->mf_import($successFlag,$tread_arr[$i]);
                     }
...
//Cams SIP import
 if($cams_auto_import[$key]=="WBR49. SIP/STP procured for the period, Request Id"){   //cams if of SIP files import;
 $semails = imap_search($inbox, 'SUBJECT "WBR49. SIP/STP procured for the period, Request Id" ');
 //echo "<pre>";print_r($semails);exit;
  ...
  $tpart=trim($this->GetBetween(' Download Link :','The zip file is protected',$ss));
...
//echo"<pre>";print_r($s_temp1);//exit;
    $slcnt=0;
    foreach($s_temp1 as $sikey=>$sival){
      $link=$s_temp1[$sikey];
        ...
        //    echo $bc_url;
         ...
         //  $unzip=true;//comment this line on server and uncomment below line on server
               ...
               if(!empty($brokerID1)) {
                ...
                $pwd2=$this->Clients_model->get_cams_RTA_import_password($brokerName);
           ...
           $unzip = $this->extract_archive($filepath,$location,$pwd2);
            if(!$unzip) {
              ...
              $today=$date->format('d-m-Y');//date('d-m-Y');
                ...
                $pSearchDate = $psearchTemp->format('d-m-Y');
        ...
        // echo "Matched\n";
         $sread_arr[]=$file;
              ...
              // echo"<pre>";print_r($sread_arr);exit;
                  ...
                  $successFlag=true;
                  $passfileType='cams_excel';
                  if(strpos($sread_arr[$i], 'downloads/SIP_files_list/cam_folder/')=== false){
                                                  $sread_arr[$i]='downloads/SIP_files_list/cam_folder/'.$sread_arr[$i];
                                                }else{
                                                  $sread_arr[$i]=$sread_arr[$i];
                                                }
                   echo "auto file=".$sread_arr[$i];
                   //echo 'SIP import';
                $allset[$key][$i]= $this->sip_import($successFlag,$sread_arr[$i]);
              }
...
//added if condition by Salmaan - 2017-11-04
if(isset($allset) && !empty($allset)) {
...
foreach($allset as $dkey=>$val){
  ...
  //   print_r($mv);
  //  $dest="/opt/lampp/htdocs/3tense_broker_new1/downloads/completed/Folio_master_list/cam_folder/";  ///opt/lampp/htdocs/3tense_broker_new1/downloads/Folio_master_list/cam_folder
  // for($i=0;$i<count($mv);$i++){
  //   $srcfile=str_replace('/opt/lampp/htdocs/3tense_broker_new1/downloads/Folio_master_list/cam_folder/','',$mv[$i]);
  //    rename($mv[$i],$dest.$srcfile);
     //unlink($mv[$i]);
     $files1 =scandir('downloads/Folio_master_list/cam_folder/');
     print_r($files1);
      foreach($mv as $key=>$val){
       $srcfile=str_replace('downloads/Folio_master_list/cam_folder/','',$mv[$key]);
       echo $srcfile;
  ...
  //    rename($mv[$i],$dest.$srcfile);
     ...
     print_r($files1);
     foreach($mv as $key=>$val){
       $srcfile=str_replace('downloads/MF_Transaction_list/cam_folder/','',$mv[$key]);
       echo $srcfile;
      $flag= in_array($srcfile,$files1);
      echo $flag;
      if($flag){
   ...
   //    rename($mv[$i],$dest.$srcfile);
      ...
      print_r($files1);
      foreach($mv as $key=>$val){
        $srcfile=str_replace('downloads/SIP_files_list/cam_folder/','',$mv[$key]);
        echo $srcfile;
       $flag= in_array($srcfile,$files1);
       echo $flag;
       if($flag){
    ...
    foreach ( $delete as $file ) {
     ...
     $rta_type="CAMS";
      ...
      $mf_error_data=$this->mf->get_all_mf_error_record($rta_type);
       ...
       $email_message.="Original Name of File:CAMS MF File=".$for_mailMfFileName.$eol;
...
if($fileType_info=="application/excel"||$fileType_info=="application/vnd.ms-excel"||$fileType_info=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                {       //for RTA excel import
                    ...
                    //check if file is empty - Salmaan - 2017-11-03
                    if(0 == filesize($tfile)) {
                        echo ' Empty file. Continuing to next...';
                        return;
                    }
                                ...
                                if(!empty($findkey=array_search('Broker Code',$dataColumns)))
                                  {
                                    $cell=$rows[$findkey];
                                      ...
                                      $arn=$broker_code;
                                      //commented below and added code - Salmaan - 2017-11-02
                                      /*$user=$this->Clients_model->get_userdata($arn);
                                    //  echo"<pre>";print_r($user);exit;
                                      if(count($user)>0 && !empty($user)){ //line changed - Salmaan - 2017-10-12 
                                      ...
                                      $brokerID = $this->session->userdata('brokerID');
                                  }
                  ...
                  if($dataRows)
                  {
                    ...
                    //var_dump($inserted);
                      if(is_array($inserted) && $inserted['code']!=0) {
                          ...
                          $this->Common_model->last_import('Client Details', null, $file, null);
                          if($uploadedStatus != 2) {
                              $uploadedStatus = 1;
...
//MF import
  function mf_import($tflag,$tfile)
    {
        ini_set('max_execution_time', 0);
        ini_set('memory_limit', '2048M');
...
//echo"<pre>";print_r($excelData);exit;
                                          ...
                                          strtoupper($dataColumns[$countCell]) == 'TRANSACTION TYPE' || strtoupper($dataColumns[$countCell]) == 'TRXN_TYPE')
                                      {
                                          if($cell || $cell != '')
                                          {
                                              ...
                                              if($mfType == "DIV" || $mfType == "DR" || $mfType == "DIR" || $mfType == "BNS" || $mfType == "BNSR" || $mfType == "DIRR") {
                                                  ...
                                                  $mfType == "CFI" || $mfType == "CFIR" || $mfType == "DMT" || $mfType == "DMTR" || $mfType == "OMTOUT" || $mfType == "OMTOUTR") {
                                                  $transactionType = "Redemption";
                                                  $mfType = "RED";
                                              } else {
                    ...
                    // echo"<pre>";print_r($dataRows);exit;
                      ...
                      //  $result = $this->mf->auto_add_import_mutual_funds($dataRows, $transID);
                        if(is_array($result)) {
                            ...
                            $valuation_done = true;
                            //var_dump($result);
                            $this->common->last_import('Mutual Fund Details', null, $tfile, null);
                            $uploadedStatus = 1;
    ...
    function sip_import($flag,$file){    //Sip import
      ini_set('max_execution_time', 0);
      ini_set('memory_limit', '2048M');
      ini_set('upload_max_filesize', '150M');
      ini_set('post_max_size', '150M');//50M
