...
$(document).ready(function () {
    ...
    $("#btn_calcSIPNeed").click(function () {
        ...
        resultMonth=need_pmt(txtCroreMonthInfRate,$("#needinstallment").val(),0,txtCroreAmount,1);       
        ...
        $('#hidden_needinterest').val(delimitNumbersValue(Math.round(txtCroreMonths)));
        $('#hidden_needFV').val(delimitNumbersValue(Math.round(resultMonth)));
        $('#hidden_chartX').val(investment);
        $('#hidden_chartY').val(txtCroreAmount - investment);
...
function needPDF(){
        ...
        var resultMonth = 0.0;
        txtCroreAmount = parseFloat($("#needSIPValue").val().replace(/,/g , ""));
        txtCroreInfRate = parseFloat($("#needrateOfInterest").val().replace(/,/g , "")); // for years
        txtCroreMonthInfRate = parseFloat($("#needrateOfInterest").val().replace(/,/g , "")); // for months
        ...
        var str1 = '<table style="width:100%;text-align: center;" >\n\
                            <tr bgcolor="#73B5EF"><th colspan="5"  align="center" >Expected Maturity Value of Your Investments</th></tr>\n\
                        \n\<tr bgcolor="#A8CFFF"> <th rowspan="2" > Investment Period <br> (in Months)</th><th colspan="4" > Expected Returns</th></tr>\n\
                            <tr  bgcolor="#A8CFFF">\n\
                                <th >8.00%  </th>\n\
                                <th>10.00% </th>\n\
                                <th>12.00% </th>\n\
        ...
        for (var i = 1; i < 7; i++) {
           ...
           var year_val= (i != 0)? result_year/12: '';
            ...
            str1 += '<tr >\n\
                        <td class="resulttable">' + delimitNumbersValue(result_year)  + yer_val + '</td>\n\
                        <td class="resulttable">' + delimitNumbersValue(result_val8)  + '</td> \n\
                        <td class="resulttable">' + delimitNumbersValue(result_val10) + '</td>\n\
                        <td class="resulttable">' + delimitNumbersValue(result_val12) + '</td>\n\
                        <td class="resulttable">' + delimitNumbersValue(result_val15) + '</td>\n\
                        \n\
                    </tr>';
