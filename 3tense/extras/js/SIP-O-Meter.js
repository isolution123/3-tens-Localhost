   ...
   $("#installment").val("300");
    ...
    $(document).ready(function(){
        ...
        $("#btn_calcSIP").click(function(){
          ...
          if ($("#rateOfReturn").val()=="0") {
           ...
           if ($("#installment").val()=="0") {
            ...
            $('#hidden_investment').val(addCommas2(investment));
            $('#hidden_earning').val(addCommas2(earnings));
            $('#hidden_FV').val(addCommas2(futurevalue));
            
           // $('#TblFutureVal').find('td remv').remove(); 
            $('.remv').remove();
            FunFillXIRR();
            $("#hidden_sip_table").val($('#TblFutureVal > tbody').html());
			
			 //code for client calc Visit strt
               $.ajax({
                        type:'POST',
                        url:'Calculators/SaveCalcVisit',
                        data:{'calcType':'SIPOMeter'},
        ...
        function FunFillXIRR()
        {
            ...
            var DefaultPeriod = [60,120,180,240,300,360,420,480,540,600,1];
